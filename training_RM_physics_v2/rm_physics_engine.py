...
from core_v2.features import get_feature_index, N_BASE
...
# ── Feature index (v2) ─────────────────────────────────────────────────
# The engine only needs ONE feature: the 1m band z-score (L3_1m_z_se_{N}).
# N for 1m comes from N_BASE['1m'] = 15 (Principle 2 tiered windows).
# This replaces v1's hardcoded `_1M_Z_IDX = 12` with a named lookup
# that won't silently break if the v2 feature layout changes.
_Z_SE_COL = f"L3_1m_z_se_{N_BASE['1m']}"
_1M_Z_IDX = get_feature_index(_Z_SE_COL)  # absolute index into FEATURE_NAMES
...
DEFAULT_NN_FILTER_ENABLED = False     # v2: no CNN trained yet — first run is unfiltered
DEFAULT_NN_MODEL_PATH = 'training_RM_physics_v2/output/pivot_direction_cnn_v2.pt'
DEFAULT_NN_TAKE_THR = 0.65        # high-confidence threshold (reused from v1)
DEFAULT_NN_FLIP_THR = 0.0         # no-FLIP mode. Only TAKE (P≥take_thr) or SKIP.
DEFAULT_MIN_RES = 1.5             # v2: raised from 0.5 — N=15 regression gives smaller sigma, so |z_se|>0.5 fires 61x more. 1.5 targets v1-comparable trade volume.
...
class RMPhysicsEngine:
    ...
    def __init__(self,
        ...
        if self.nn_filter_enabled:
            from training_RM_physics_v2.nn_direction import load_nn_filter
        ...
        if (pivot_confirmed and pivot_idx is not None
            ...
            if not np.isnan(r_at_pivot) and abs(r_at_pivot) >= self.min_res_strength:
                ...
                if self.nn_filter_enabled and self._nn_model is not None:
                    from training_RM_physics_v2.nn_direction import predict_pwin
                    try:
