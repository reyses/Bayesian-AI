...
## 2026-03-09 - [Tkinter Tooltip Flashing & UX]
**Learning:** Tooltips in high-density Tkinter dashboards (like `visualization/dashboard.py`) can cause aggressive flashing and UI obstruction if they appear instantly.
**Action:** Implement a ~500ms delay (`after`), an exit cancellation (`after_cancel`), and a `<ButtonPress>` binding to dismiss the tooltip, ensuring smooth and unobtrusive interactions.
