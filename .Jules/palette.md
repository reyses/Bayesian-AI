...
## 2026-03-11 - [Tooltip Delay in High-Density UI]
**Learning:** Immediate tooltip triggers create visual noise and distract users in high-density dashboards when quickly moving the mouse across multiple elements.
**Action:** Add a 500ms delay before showing tooltips, ensure active scheduled events are cancelled on `<Leave>`, and bind `<ButtonPress>` to dismiss.
