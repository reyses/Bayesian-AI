"""
NMP Overshoot — NMP base, hold through mean for momentum overshoot.

Same NMP entry (z extreme + vr < 1.0 → fade) but:
  Entry: base NMP only (no wick filter — overshoot doesn't need rejection)
  Exit:  hold UNTIL velocity fades AND price passed through mean
         OR z reaches opposite extreme (overshoot completed)

Physics: OU with residual momentum. Price reverts to mean but velocity
at mean crossing is nonzero → overshoots to opposite side. The exit
waits for momentum exhaustion, not just mean arrival.

Second ExNMP — tests the overshoot hypothesis.
