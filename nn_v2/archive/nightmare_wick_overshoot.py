"""
NMP Wick Overshoot — kill shot entry + overshoot exit + breakeven regret.

Entry: NMP + 5m_wick > 0.83 + 15m_wick > 0.77 (same as kill shot)
Exit:  opposite z extreme (z flipped sign, |z| > 1.0)
       OR momentum exhausted (|velocity| < 0.3 AND p_center > 0.60)

After each trade closes, computes breakeven regret: scans forward from
entry until price returns to entry price. This measures the full extent
of the move — peak PnL, bars to peak, bars to breakeven.
...
P_CENTER_THROUGH = 0.60    # price must have passed through mean
Z_OPPOSITE_EXTREME = 1.0   # z flipped sign and reached this = overshoot done

# Absolute indices for overshoot features
    ...
    def on_state(self, state: Dict):
        ...
        if self.in_pos:
            ...
            if is_1m_boundary:
                ...
                # Exit 1: momentum exhausted AND price passed through mean
        ...
        # === ENTRY CHECK — NMP + wick rejection filter, 1m boundaries only ===
