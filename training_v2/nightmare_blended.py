...
# ─────────────────────────────────────────────────────────────────────────
# V2 MIGRATION — calibrated thresholds (see reports/findings/v1_v2_threshold_calibration/)
# z_se thresholds: ~10-15% lower because V2 z_se distribution is narrower
# velocity thresholds: 3-5x smaller because V2 L2_velocity_w is windowed slope
#                      (V1 was OLS slope on cuda — different magnitude)
# variance_ratio, wick_ratio, vol_rel: UNCHANGED (machine-precision parity via
# core_v2.v1_compat shim used by tools/build_v2_to_v1_compat_cache.py)
# ─────────────────────────────────────────────────────────────────────────

# NMP entry  (V1 ROCHE=2.0 -> V2 calibrated 1.87)
ROCHE = 1.87
VR_ENTRY = 1.0
...
# MTF_EXHAUSTION entry  (V1 30/10 -> V2 calibrated 8.44/2.47, 3-4x smaller)
MTF_5M_VEL_MIN = 8.44      # 5m |velocity| must be above this  (V1: 30.0)
MTF_5M_DECEL = 0.50
MTF_1M_VEL_ALIVE = 2.47    # 1m |velocity| must show life  (V1: 10.0)
...
FREIGHT_TRAIN_THRESHOLD = 20.0  # |1m_velocity| above this = always ride  (V1: 100.0)
                                # 20 ~= 2x VELOCITY_THRESHOLD (V1 ratio preserved)
...
# EXHAUSTION_BAR  (V1 z 1.4 -> V2 calibrated 1.32 at 1m)
EXHAUST_Z_MIN = 1.32              # entry: must be deep in z extreme  (V1: 1.4)
...
# MTF_EXHAUSTION  (V1 z 1.4 -> V2 calibrated 1.32-1.18 across TFs)
MTF_Z_MIN = 1.32                  # entry: must be deep in z extreme  (V1: 1.4)
...
# Cascade: 1h z alignment  (V1 1.0 -> V2 calibrated 0.87 at 1h)
H1_Z_MIN = 0.87
...
H1_AGAINST_Z_MIN = 1.40  # V2 calibrated (V1: 1.5). |1h_z| extreme threshold.
...
CNN_PER_TIER_DIR = 'training_v2/output/nn'
