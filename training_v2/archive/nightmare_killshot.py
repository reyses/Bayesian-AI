"""
NMP Kill Shot — NMP with wick rejection filter + p_at_center exit.

Same NMP core (z extreme + vr < 1.0 → fade) but:
  Entry: adds 5m_wick_ratio > 0.83 AND 15m_wick_ratio > 0.77
         (multi-TF rejection confirms reversion setup)
  Exit:  1m_p_at_center > 0.60 (price reached regression center)
         instead of z < 0.5

This is the first ExNMP — NMP base with physics-derived quality filter.
Used to validate kill shot conditions and model the pattern for NN.
...
# Kill shot exit
P_CENTER_EXIT = 0.60 # 1m p_at_center threshold (price reached regression center)
