"""
NMP Resonance Cascade — multi-TF aligned entry + p_at_center exit.

Entry: NMP base + wick rejection + 1h z alignment.
  All regression bands must agree: 1m z extreme, 5m/15m wick rejection,
  AND 1h z extreme in the same direction. Multi-TF resonance.

Exit: 1m_p_at_center > 0.60 (same as kill shot — can upgrade later)

Physics: when regression bands at multiple TFs all point the same way,
the reversion force is amplified. Fewer trades, higher confidence,
bigger PnL per trade.

Third ExNMP — resonance cascade.
...
# Wick rejection filter (same as kill shot)
WICK_5M_MIN = 0.83   # 5m wick ratio must exceed (rejection candle)
WICK_15M_MIN = 0.77  # 15m wick ratio must exceed (multi-TF confirmation)

# Cascade: 1h z must align with trade direction
H1_Z_MIN = 1.0       # |1h_z_se| must exceed this for resonance

# Exit
P_CENTER_EXIT = 0.60 # 1m p_at_center threshold

# 1h z_se absolute index in 79D
_1H_Z_IDX = 40
...
# Absolute indices for kill shot features (from 79D layout)
_5M_WICK_IDX = 68    # 5m_wick_ratio
_15M_WICK_IDX = 71   # 15m_wick_ratio
...
class CascadeEngine:
    ...
    def on_state(self, state: Dict):
        ...
        if self.in_pos:
            ...
            # Kill shot exit: p_at_center > threshold — only on 1m boundaries
            if is_1m_boundary:
                p_center = feat[_1M_P_CENTER_IDX]
                if p_center > P_CENTER_EXIT:
                    self._close_trade(price, ts, time_str, 'center_reached', feat, s1m)

        # === ENTRY CHECK — NMP + wick + 1h resonance, 1m boundaries only ===
        if not self.in_pos and is_1m_boundary:
            wick_5m = feat[_5M_WICK_IDX]
            wick_15m = feat[_15M_WICK_IDX]
            h1_z = feat[_1H_Z_IDX]

            if (abs(z) > ROCHE and vr < VR_ENTRY and
                    wick_5m > WICK_5M_MIN and wick_15m > WICK_15M_MIN):
                direction = 'short' if z > 0 else 'long'

                # Cascade: 1h z must support the trade direction
                # Long trade: 1h z should be negative (price below 1h band)
                # Short trade: 1h z should be positive (price above 1h band)
