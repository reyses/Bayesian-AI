...
Flags (all opt-in):
  ...
  --cascade         FORWARD-PASS MODE: switch from isolated (per-tier parallel
                    engines) to single-engine cascade using TIER_PRIORITY
                    first-match-wins. This mirrors live/blended behavior —
                    contested bars go to highest-priority tier, not all tiers.
  --oos             Run against 2026 days only (OOS subset). Writes iso_oos.pkl.
  --with-oos        Run IS first, then OOS automatically. Writes BOTH pickles.
  --log [PATH]      Tee stdout to a file. Without PATH, auto-saves to
                    reports/findings/iso_run_<target>_<timestamp>.txt.
...
def main():
    ...
    # reports/findings/iso_run_<target>_<YYYYMMDD_HHMMSS>.txt.
    log_path = None
    if '--log' in args:
        idx = args.index('--log')
    ...
    def _phase1_one_target(tgt):
        ...
        prefix = 'iso_oos' if tgt == 'oos' else 'iso_is'
        if only_tiers:
        ...
        if trades_:
            ...
            print(f'  Wrote: training_iso/output/trades/{pkl_name_}')
