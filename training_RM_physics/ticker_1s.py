"""
1-second ticker for realistic backtesting.
...
Yields per-1s state dicts compatible with engine `on_state(state)`:
  ...
  state = {
    ...
    'features':  np.ndarray,   # 79D features at nearest-past 5s row
    'bar_data':  dict | None,  # MOST-RECENTLY-COMPLETED 1m bar (no lookahead)
    'bar_idx':   int,
  }

Timing convention (critical — no lookahead):
  - Price, high, low are for the CURRENT 1s bar (known at time of tick).
  - Features use `searchsorted(feat_ts, ts, right) - 1` → feature row with
    timestamp <= current 1s ts. Nearest-past, no lookahead.
  - bar_data refers to the most recently COMPLETED 1m bar, i.e. the 1m
    bar whose interval end (open_ts + 60) <= current_ts. Its close is
    historical, known when we're in the tick.
  - When current_ts crosses a 1m boundary, the 1m bar that just ended
    becomes the bar_data. Engine detects this via bar_data.timestamp
    changing (same API as FeatureTicker).

Source files:
  - 1s OHLCV:   DATA/ATLAS/1s/<day>.parquet
  - 5s features: DATA/ATLAS/FEATURES_5s/<day>.parquet
  - 1m OHLCV:   DATA/ATLAS/1m/<day>.parquet

Not yielded during warm-up: we drop 1s ticks before the first complete 1m
bar to avoid an initial NaN/None state.
"""
import os
import sys
...
class OneSecondTicker:
    """Yields per-1s state with 5s features (nearest-past) and most-recently-
    completed 1m bar_data. No lookahead."""

    def __init__(self, day: str,
                 ps_dir: str = 'DATA/ATLAS/1s',
                 feat_dir: str = 'DATA/ATLAS/FEATURES_5s',
                 p1m_dir: str = 'DATA/ATLAS/1m'):
        self.day = day
        p1s_path = os.path.join(ps_dir, f'{day}.parquet')
        feat_path = os.path.join(feat_dir, f'{day}.parquet')
        ...
        feat_cols = [c for c in df_feat.columns if c in FEATURE_NAMES]
        self._feat_matrix = df_feat[feat_cols].values.astype(np.float32)
