...
Flags (all opt-in):
  ...
  --chains N        Enable chain positions up to N (default 1 — small equity)
  --oos             Run against 2026 days only. Writes rm_oos.pkl.
  --with-oos        Run IS first, then OOS automatically. Writes BOTH pickles.
  --log [PATH]      Tee stdout to a file.

Pickle output naming:
  IS forward pass  →  training_RM_physics/output/trades/rm_is.pkl
  OOS forward pass →  training_RM_physics/output/trades/rm_oos.pkl

Usage:
    python training_RM_physics/run_rm.py                # default Phase-1 IS
    python training_RM_physics/run_rm.py --with-oos     # IS + OOS
    python training_RM_physics/run_rm.py --chains 3     # concurrent positions
    python training_RM_physics/run_rm.py --oos          # OOS only
"""
import os
import sys
import glob
...
OUTPUT_DIR = 'training_RM_physics/output'
...
def run_iso_forward(target='is', only_tiers=None, max_chains=4, cascade=False):
    ...
    roll up cleanly.
    """
    from training_RM_physics.rm_physics_engine import IsoEngine, TIER_PRIORITY
...
def run_regret(pkl_name: str = 'rm_is.pkl'):
    ...
    """
    from training_RM_physics.regret import compute_all_regrets, correct_trades
...
def main():
    ...
    """
    from training_RM_physics.rm_physics_engine import TIER_PRIORITY
    args = sys.argv[1:]
    ...
    def _phase1_one_target(tgt):
        ...
        if trades_:
            ...
            print(f'  Wrote: training_RM_physics/output/trades/{pkl_name_}')
