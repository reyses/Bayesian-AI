...
def test_swing_noise_numba_optimization():
    ...
    # Numba JIT (includes first-run compilation if not cached)
    ...
    # Assert correctness
    assert np.allclose(res_np, res_numba, rtol=1e-4), "Numba output does not match NumPy output"

    print(f"\nNumPy time: {np_time:.4f}s")
