...
> Last updated: Auto-generated. CUDA-only (CPU fallback removed).
...
| `core/fractal_clustering.py` |  649  | `FractalClusteringEngine` | Recursive K-Means → 100-1000 templates with per-template OLS/logistic models |
| `core/execution_engine.py` |  1332  | `ExecutionEngine` | Gate cascade (0-4), direction cascade, sizing. Single source for IS/OOS/live |
| `core/exit_engine.py` |  454  | `ExitEngine`, `PositionState`, `make_position()` | Unified exit cascade: SL→TP→BandUrgent→EnvelopeDecay→PeakGiveback→BreakevenLock→BeliefFlip→Hold |
| `core/feature_extraction.py` |  53  | `extract_feature_vector()` | Canonical 16D feature vector (single source of truth) |
| `core/timeframe_belief_network.py` |  1256  | `TimeframeBeliefNetwork` | 11 TF workers (1s→1D), path conviction, band confluence, direction models |
...
| File | Lines | Purpose |
|------|------:|---------|
| `training/trainer.py` |  6163  | Main entry point: 6-phase pipeline (discovery→clustering→optimization→IS→OOS→strategy) |
