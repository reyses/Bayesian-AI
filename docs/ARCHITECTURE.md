...
| `core/execution_engine.py` | 1146 | `ExecutionEngine` | Gate cascade (0-4), direction cascade, sizing. Single source for IS/OOS/live |
...
| `core/timeframe_belief_network.py` | 1162 | `TimeframeBeliefNetwork` | 11 TF workers (1s→1D), path conviction, band confluence, direction models |
...
|------|------:|------------|---------|
| `live/live_engine.py` | 1741 | `LiveEngine` | Main live trading loop — mirrors Phase 4 on real-time NT8 feed |
| `live/history_replay.py` | 737 | `HistoryReplayEngine` | Compressed forward pass over ATLAS to warm up brain/TBN/exits |
...
| `live/launcher.py` | 261 | `main()` | Entry point for live mode |

## Training

| File | Lines | Purpose |
|------|------:|---------|
| `training/trainer.py` | 5180 | Main entry point: 6-phase pipeline (discovery→clustering→optimization→IS→OOS→strategy) |
...
## Total: ~19,784 lines across core/, live/, training/, visualization/
