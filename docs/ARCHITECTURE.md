...
> Last updated: 2026-03-21. CUDA-only (CPU fallback removed).
...
| `core/exit_engine.py` | 643 | `ExitEngine`, `PositionState`, `make_position()` | Unified exit cascade: SL→TP→BandUrgent→EnvelopeDecay→PeakGiveback→BreakevenLock→BeliefFlip→Hold |
...
|------|------:|------------|---------|
| `live/live_engine.py` | 1881 | `LiveEngine` | Main live trading loop — mirrors Phase 4 on real-time NT8 feed |
...
| File | Lines | Purpose |
|------|------:|---------|
| `training/trainer.py` | 5273 | Main entry point: 6-phase pipeline (discovery→clustering→optimization→IS→OOS→strategy) |
...
## Total: ~20900 lines across core/, live/, training/, visualization/
