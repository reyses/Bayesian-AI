...
> Last updated: 2026-03-16. CUDA-only (CPU fallback removed).
...
| `core/execution_engine.py` | 1369 | `ExecutionEngine` | Gate cascade (0-4), direction cascade, sizing. Single source for IS/OOS/live |
| `core/exit_engine.py` | 569 | `ExitEngine`, `PositionState`, `make_position()` | Unified exit cascade: SL→TP→BandUrgent→EnvelopeDecay→PeakGiveback→BreakevenLock→BeliefFlip→Hold |
| `core/feature_extraction.py` | 53 | `extract_feature_vector()` | Canonical 16D feature vector (single source of truth) |
| `core/timeframe_belief_network.py` | 1354 | `TimeframeBeliefNetwork` | 11 TF workers (1s→1D), path conviction, band confluence, direction models |
...
| File | Lines | Purpose |
|------|------:|---------|
| `training/trainer.py` | 6649 | Main entry point: 6-phase pipeline (discovery→clustering→optimization→IS→OOS→strategy) |
...
## Total: ~29,393 lines across core/, live/, training/, visualization/
