...
> Last updated: 2026-03-29. CUDA-only (CPU fallback removed).
...
|------|------:|------------|---------|
| `live/live_engine.py` | 2878 | `LiveEngine` | Main live trading loop — mirrors Phase 4 on real-time NT8 feed |
...
| `live/launcher.py` | 343 | `main()` | Entry point for live mode |

## Training

| File | Lines | Purpose |
|------|------:|---------|
| `training/trainer.py` | 5317 | Main entry point: 6-phase pipeline (discovery→clustering→optimization→IS→OOS→strategy) |
...
## Total: ~40,193 lines across core/, live/, training/, visualization/
