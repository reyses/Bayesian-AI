...
> Auto-generated by Jules on 2026-03-07. Do not edit manually.

## System Overview
Bayesian-AI is a physics-based, fractal trading system that models the market as a three-body problem (Fair Value, +2σ Resistance, -2σ Support). It uses a "Bayesian Brain" to learn the win probability of specific market states (patterns) and a "Fractal DNA" system to align signals across 8 timeframe levels (1h down to 15s).
...
|------|-----------------|------|
| `core/statistical_field_engine.py` | `StatisticalFieldEngine` | Computes 3-body state, forces, and wave function |
| `core/bayesian_brain.py` | `BayesianBrain` | Hash-map learning engine (State -> WinProb) |
| `core/market_state.py` | `MarketState` | Dataclass defining the full 16D market state |
...
```mermaid
graph TD
    A[Raw Parquet] --> B[StatisticalFieldEngine]
    B --> C[MarketState]
