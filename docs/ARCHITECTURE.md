...
> Last updated: 2026-03-15. CUDA-only (CPU fallback removed).
...
| `core/execution_engine.py` | 1339 | `ExecutionEngine` | Gate cascade (0-4), direction cascade, sizing. Single source for IS/OOS/live |
| `core/exit_engine.py` | 560 | `ExitEngine`, `PositionState`, `make_position()` | Unified exit cascade: SL→TP→BandUrgent→EnvelopeDecay→PeakGiveback→BreakevenLock→BeliefFlip→Hold |
| `core/feature_extraction.py` | 53 | `extract_feature_vector()` | Canonical 16D feature vector (single source of truth) |
| `core/timeframe_belief_network.py` | 1340 | `TimeframeBeliefNetwork` | 11 TF workers (1s→1D), path conviction, band confluence, direction models |
...
|------|------:|------------|---------|
| `live/live_engine.py` | 1954 | `LiveEngine` | Main live trading loop — mirrors Phase 4 on real-time NT8 feed |
| `live/history_replay.py` | 559 | `HistoryReplayEngine` | Compressed forward pass over ATLAS to warm up brain/TBN/exits |
| `live/atlas_loader.py` | 143 | `load_atlas_range()` | Parquet I/O for ATLAS multi-TF dataset |
| `live/bar_aggregator.py` | 357 | `LiveBarAggregator` | 1s→15s OHLCV aggregation with state recomputation |
...
| File | Lines | Purpose |
|------|------:|---------|
| `training/trainer.py` | 6543 | Main entry point: 6-phase pipeline (discovery→clustering→optimization→IS→OOS→strategy) |
...
| File | Lines | Purpose |
|------|------:|---------|
| `visualization/dashboard.py` | 1499 | Tkinter "Fractal Command Center" (1600x950): training Pareto + live price/PnL |
...
## Total: ~29,200 lines across core/, live/, training/, visualization/
