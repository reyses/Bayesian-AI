...
> Last updated: 2026-03-20. CUDA-only (CPU fallback removed).
...
| `core/exit_engine.py` | 630 | `ExitEngine`, `PositionState`, `make_position()` | Unified exit cascade: SL→TP→BandUrgent→EnvelopeDecay→PeakGiveback→BreakevenLock→BeliefFlip→Hold |
| `core/feature_extraction.py` | 53 | `extract_feature_vector()` | Canonical 16D feature vector (single source of truth) |
| `core/timeframe_belief_network.py` | 1488 | `TimeframeBeliefNetwork` | 11 TF workers (1s→1D), path conviction, band confluence, direction models |
...
| File | Lines | Purpose |
|------|------:|---------|
| `training/trainer.py` | 6070 | Main entry point: 6-phase pipeline (discovery→clustering→optimization→IS→OOS→strategy) |
...
| File | Lines | Purpose |
|------|------:|---------|
| `visualization/dashboard.py` | 1751 | Tkinter "Fractal Command Center" (1600x950): training Pareto + live price/PnL |
...
## Total: ~30016 lines across core/, live/, training/, visualization/
