...
> Last updated: 2026-03-17. CUDA-only (CPU fallback removed).
...
| `core/timeframe_belief_network.py` |  1408 | `TimeframeBeliefNetwork` | 11 TF workers (1s→1D), path conviction, band confluence, direction models |
...
|------|------:|------------|---------|
| `live/live_engine.py` |  1649 | `LiveEngine` | Main live trading loop — mirrors Phase 4 on real-time NT8 feed |
...
| File | Lines | Purpose |
|------|------:|---------|
| `training/trainer.py` |  6801 | Main entry point: 6-phase pipeline (discovery→clustering→optimization→IS→OOS→strategy) |
...
| File | Lines | Purpose |
|------|------:|---------|
| `visualization/dashboard.py` |  1665 | Tkinter "Fractal Command Center" (1600x950): training Pareto + live price/PnL |
...
## Total: ~29,757 lines across core/, live/, training/, visualization/
