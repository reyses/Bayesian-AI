...
| `core/fractal_clustering.py`  | 663 | `FractalClusteringEngine` | Recursive K-Means → 100-1000 templates with per-template OLS/logistic models |
...
| `core/timeframe_belief_network.py`  | 1262 | `TimeframeBeliefNetwork` | 11 TF workers (1s→1D), path conviction, band confluence, direction models |
...
| File | Lines | Purpose |
|------|------:|---------|
| `training/trainer.py`  | 6362 | Main entry point: 6-phase pipeline (discovery→clustering→optimization→IS→OOS→strategy) |
