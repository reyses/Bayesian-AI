...
> Last updated: 2026-04-04. CUDA-only (CPU fallback removed).
...
| `core/fractal_clustering.py` | 824 | `FractalClusteringEngine` | Recursive K-Means → 100-1000 templates with per-template OLS/logistic models |
| `core/execution_engine.py` | 1418 | `ExecutionEngine` | Gate cascade (0-4), direction cascade, sizing. Single source for IS/OOS/live |
| `core/exit_engine.py` | 640 | `ExitEngine`, `PositionState`, `make_position()` | Unified exit cascade: SL→TP→BandUrgent→EnvelopeDecay→PeakGiveback→BreakevenLock→BeliefFlip→Hold |
| `core/feature_extraction.py` | 249 | `extract_feature_vector()` | Canonical 16D feature vector (single source of truth) |
| `core/timeframe_belief_network.py` | 1523 | `TimeframeBeliefNetwork` | 11 TF workers (1s→1D), path conviction, band confluence, direction models |
...
|------|------:|------------|---------|
| `live/live_engine.py` | 2944 | `LiveEngine` | Main live trading loop — mirrors Phase 4 on real-time NT8 feed |
| `live/history_replay.py` | — | `HistoryReplayEngine` | Compressed forward pass over ATLAS to warm up brain/TBN/exits |
| `live/atlas_loader.py` | — | `load_atlas_range()` | Parquet I/O for ATLAS multi-TF dataset |
| `live/bar_aggregator.py` | 406 | `LiveBarAggregator` | 1s→15s OHLCV aggregation with state recomputation |
| `live/nt8_client.py` | 221 | `NT8Client` | Asyncio TCP bridge to NinjaTrader 8 BayesianBridge indicator |
| `live/order_manager.py` | 353 | `OrderManager` | Order lifecycle, position shadow, trade CSV logging |
| `live/session_tracker.py` | 271 | `SessionTracker` | Session PnL, drawdowns, trade log, session reports |
...
| `live/launcher.py` | 351 | `main()` | Entry point for live mode |

## Training

| File | Lines | Purpose |
|------|------:|---------|
| `training/trainer.py` | 5442 | Main entry point: 6-phase pipeline (discovery→clustering→optimization→IS→OOS→strategy) |
...
## Total: ~44,630 lines across core/, live/, training/, visualization/
