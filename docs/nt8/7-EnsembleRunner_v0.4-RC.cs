//
// 7-EnsembleRunner_v0.3-RC.cs
// -----------------------------------------------------------------------------
// NinjaTrader 8 STRATEGY: Ensemble entry combiner + R-trigger ride-only exit.
// v0.3-RC -- WRAPPER BUGFIX: warm the R-trigger with a prior-day 5s tail (P2-12).
//
// WHY v0.3 (research/nt8_port P2c, root-cause of "R-trigger fired ZERO times"):
//   The v0.2 backtest (2026-06-22..07-17, 44 trades) fired the RTriggerReversal
//   exit ZERO times -- every exit was a catastrophic stop or a session close --
//   even though the golden vectors carry ~19.5 confirmed reversals/day. The
//   SHARED-CORE is NOT at fault: an empirical minute-by-minute replay
//   (research/nt8_port/tools/p2c_rtrigger_replay.py) proves the batch core is
//   perfectly TRUNCATION-INVARIANT (0/406 mismatches when the day-so-far is fed
//   with the harness tail) and the zz_confirm sign convention is correct
//   (flip=-1 == new down-leg == "against a long" == -openDir). The bug is the
//   WRAPPER's WARMUP: v0.2 cleared the 5s buffer to EMPTY each session and hard-
//   wired Ctx.Start=0, so the R-trigger's reversal threshold min_rev = round(
//   ATR(14 1m)x4 / TICK) was computed COLD on the volatile RTH-open bars instead
//   of the prior-day-warmed ATR the golden/parity used. Empirically that inflates
//   min_rev 2.3-2.6x (119->272t on 2024-06-26; 167->428t on 2025-10-16) and cuts
//   confirmed reversals to ~1/3 (19->6; 40->7 per day). The few survivors are then
//   pre-empted by the every-5s catastrophic-stop poll (the backtest ran it ON,
//   against the SIM default) and by NT8's session template auto-closing at 14:00
//   before the 15:55 flatten -- collapsing RTrigger fires to zero. WarmupTailBars5s
//   existed in v0.2 but was DEAD CODE (never consumed). FIX: seed the tail.
//
...
//   Guards : optional catastrophic stop (default OFF in SIM; polled every 5s bar --
//            see stop-race note at OnBarUpdate); flatten at 15:55 CT (but note NT8's
//            session template may auto-close earlier, e.g. 14:00 -- P2-5/P2-11).
//
...
// VERSION: 0.4.0-RC
// -----------------------------------------------------------------------------
// v0.4.0-RC (2026-07-19): CATASTROPHIC STOP -> NATIVE RESTING ORDER (P2-10).
//   Moises observed stop rule-violations (fills 10-230% past the point setting)
//   and diagnosed the cause as the stop being reached only after a bar-cadence
//   wait. Correct: the v0.3 stop was a POLLED check on bar close + a MARKET
//   flatten that filled at the NEXT bar's open -> unbounded slip that scales
//   with bar size. FIX: SetStopLoss(CalculationMode.Ticks) attaches a real
//   resting stop order to each entry; NT8 fills it INTRABAR the instant price
//   touches the level, on ANY bar size, with bounded slip. This removes the
//   cadence dependence entirely (the reason changing the anchor TF is NOT the
//   fix -- a polled stop on 1m waits a full minute; a resting stop waits 0).
//   The old intrabar poll is removed to avoid a double-stop. Decision core
//   (SHARED-CORE-V02 region) is byte-identical to v0.2/v0.3 (verify_region PASS).
// -----------------------------------------------------------------------------
...
{
    public class EnsembleRunner_v04 : Strategy
    {
        private const string VERSION = "0.4.0-RC";
        ...
        protected override void OnStateChange()
        {
            ...
            else if (State == State.Configure)
            {
                ...
                // ---- v0.4: native resting catastrophic stop (P2-10) ----
                // Fills INTRABAR the instant price touches the level (any bar size,
                // bounded slip) -- replaces the v0.3 polled market flatten. Attached
                // to both entry signal names; NT8 manages the OCO with the exits.
                if (EnableCatastrophicStop)
                {
                    int stopTicks = (int)Math.Round(CatastrophicStopPoints / TICK);
        ...
        protected override void OnBarUpdate()
        {
            ...
            // ---- catastrophic stop: now a NATIVE RESTING ORDER (v0.4, P2-10 RESOLVED) ----
            // The v0.3 polled market flatten lived here and caused 10-230% slip past the
            // point setting (fired only on bar close, filled at the next bar's open). It is
            // replaced by SetStopLoss(...) attached in State.Configure, which NT8 fills
            // INTRABAR at the stop price with bounded slip on any bar size -- no cadence
            // wait. Nothing to poll here anymore. (Moises fix 2026-07-19.)

            // ---- session flatten guard (15:55 CT): flatten + block new entries ----
            // SESSION-CLOSE MISMATCH (v0.3, P2-5/P2-11): this 15:55 CT guard is frequently NEVER
            //   reached because NT8's trading-hours template auto-flattens earlier -- the v0.2
            //   backtest showed "Exit on session close" fills at 14:00 CT (IsExitOnSessionClose
            //   Strategy + the instrument's session template), well before 15:55. Until the
            //   America/Chicago session calendar is pinned (P2-5) and the flatten/close policy is
            //   reconciled (P2-11), positions may be force-closed at the template boundary rather
            //   than by this guard or by the R-trigger. Left AS-IS pending P2-5/P2-11.
        ...
        private void RunDecision(long curMin, DateTime now)
        {
            ...
            List<BarRec> recs = global::NinjaTrader.NinjaScript.Strategies.EnsembleV02Core.Core.ProcessDay(x, tmpl);
        ...
        // v0.3: keep only the last `keep` buffered 5s bars as the new session's warmup
        // tail (mirrors harness `df.tail(TAIL)`). Trims every parallel buffer in lockstep.
        private void TrimBuffersToTail(int keep)
        {
            int n = bTs.Count;
            int drop = n - keep;
            if (drop <= 0) return;                          // < tail worth of history: keep all
            bTs.RemoveRange(0, drop);  bO.RemoveRange(0, drop);  bH.RemoveRange(0, drop);
            bL.RemoveRange(0, drop);   bC.RemoveRange(0, drop);  bV.RemoveRange(0, drop);
            bRth.RemoveRange(0, drop); bB9.RemoveRange(0, drop); bTod.RemoveRange(0, drop);
            bZse.RemoveRange(0, drop);
        }
        ...
        private void RollSession(DateTime t)
        {
            if (t.Date != sessionDate)
            {
                ...
                pdHasBars = false;
                // v0.3 FIX (P2-12, the R-trigger warmup root-cause): the parity harness feeds a
                //   prior-day 5s TAIL (build_ctx: `full = concat([tail, df]); start = len(tail)`,
                //   TAIL == Start ~ 2500) so ATR(14 1m) -- and therefore the R-trigger reversal
                //   threshold min_rev = round(zz_thr[first_rth]/TICK) -- is WARM at RTH open.
                //   v0.2 cleared the buffer to EMPTY and set dayStartIdx=0, computing min_rev COLD
                //   on the volatile open (proven 2.3-2.6x inflated -> confirmed reversals cut to
                //   ~1/3 -> RTriggerReversal never fires; see research/nt8_port/reports/
                //   p2c_rtrigger_bug.md). WarmupTailBars5s existed but was DEAD CODE.
                //   FIX: RETAIN the last WarmupTailBars5s buffered 5s bars as the new session's
                //   warmup tail and set Ctx.Start = that retained count. ProcessDay emits BarRecs
                //   only for i >= Start (current-session RTH), so the tail warms the generators /
                //   ATR / R-trigger WITHOUT being traded -- exactly the harness semantics.
                //   NOTE: exact BIT-parity of the tail vs the Databento export remains P2-12 (the
                //   tail bars now come from the NT8 tape, not the golden parquet); this fix
                //   restores FUNCTIONAL warmth (real prior-day ATR, not a cold intraday estimate).
                TrimBuffersToTail(WarmupTailBars5s);
                dayStartIdx = bTs.Count;   // Ctx.Start -> first bar of the NEW session
            }
...
// -----------------------------------------------------------------------------
// CHANGELOG
//   v0.3.0-RC (2026-07-18, research/nt8_port P2c): WRAPPER BUGFIX -- "R-trigger
//     fired ZERO times." Root-caused empirically (p2c_rtrigger_replay.py over the
//     golden days) and fixed WRAPPER-ONLY; the SHARED-CORE-V02 region is byte-
//     identical to v0.2 (verify_region.py PASS).
//       ROOT CAUSE (P2-12 warmup): v0.2 cleared the 5s buffer to EMPTY each session
//         and hardwired Ctx.Start=0, so the R-trigger threshold min_rev = round(
//         ATR(14 1m)x4 / TICK) was computed COLD on the RTH open instead of on the
//         prior-day-warmed ATR the golden/parity used. Proven: min_rev inflates
//         2.3-2.6x (119->272t, 167->428t) and confirmed reversals drop to ~1/3
//         (19->6, 40->7 per day); with a per-day replay the WARM core is perfectly
//         truncation-invariant (0/406 mismatches) and the sign convention is correct,
//         so the fault is entirely the wrapper's missing warmup. WarmupTailBars5s was
//         declared but DEAD CODE.
//       FIX: RollSession now RETAINS the last WarmupTailBars5s buffered 5s bars as the
//         new session's warmup tail (new TrimBuffersToTail helper) and sets
//         dayStartIdx = retained count, i.e. Ctx.Start -> first current-session bar.
//         ProcessDay still emits/decides only on i >= Start (current RTH), so the tail
//         warms ATR / generators / R-trigger without being traded -- harness semantics.
//         Empirically restores confirmed reversals to ~19-40/day and makes the R-trigger
//         exit reachable at every minute the full-day run confirms a reversal.
//       ALSO documented (no behavior change): the stop-race (5s stop poll pre-empts the
//         once-per-minute R-trigger when the stop is ON -- keep it OFF in SIM), the stop
//         residual-slip expectation (market fill next 5s bar; native stop = P2-10), and
...
//   P2-12 [PARTIALLY FIXED in v0.3] warmup tail now SEEDED (functional warmth); exact
//         BIT-parity of the tail + prior-daily profile vs the harness Start remains open.
