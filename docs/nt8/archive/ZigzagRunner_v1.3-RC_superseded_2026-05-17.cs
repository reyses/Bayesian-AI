// =============================================================================
// ZigzagRunner 1.3-RC -- 2026-04-26  (multi-TF: 3 configurable TFs for risk)
// =============================================================================
...
//     Tier1 trail floor.
//
//   * REFACTOR: DynamicRiskManager_v13.EvaluateStopState_v13() split into
...
//
//   * StagnationMonitor_v13: still per-trade-recreated, evaluated on the
...
//   2. Apply ZigzagRunner_v1.3-RC to ANY chart — primary TF is your
...
{
    public class ZigzagRunner_v13 : Strategy
    {
        // ── Settings ─────────────────────────────────────────────────────
        ...
        // ── Version ──────────────────────────────────────────────────────
        private const string VERSION = "1.3-RC";
        ...
        private DynamicRiskManager_v13 riskMgr;
        private StagnationMonitor_v13 stagnationMon;
        ...
        protected override void OnStateChange()
        {
            ...
            {
                Name                             = "ZigzagRunner_v1.3-RC";
                Description                      = "Multi-TF zigzag pivots + 3-TF risk machinery (Pivot/SL/Trail) + StagnationMonitor_v13 + CSV ledger. v" + VERSION;
            ...
            else if (State == State.Configure)
            {
                ...
                // Add the three secondary series. Order matters: their
                // BarsInProgress index follows the call order below.
                ...
                riskMgr = new DynamicRiskManager_v13(
                    HardStopLossPoints,
                    Tier1ActivatePoints, Tier1TrailDistancePoints,
                    Tier2ActivatePoints, Tier2TrailPercent,
                    RouteStopOrder);
                // Wire NT8's Print method as the DRM diagnostic logger so
                // every state transition and stop computation lands in the
                // NinjaScript Output panel. Comment out if Output spam is
                // unwanted; logger == null = silent.
                riskMgr.SetDiagLogger(s => Print("[v1.3-RC " + Time[0].ToString("HH:mm:ss") + "] " + s));
                stagnationMon = new StagnationMonitor_v13(MaxNegativeBars);
                EnsureCsvHeader();
            }
        }
        ...
        private void EnsureCsvHeader()
        {
            ...
            {
                Print("ZigzagRunner v1.3-RC CSV init error: " + ex.Message);
            }
        }
        ...
        private void AppendTradeCsv(DateTime exitTime, string exitReason, double exitPrice, int qty)
        {
            ...
            {
                Print("ZigzagRunner v1.3-RC CSV append error: " + ex.Message);
            }
        }
                                                  ...
                                                  DateTime time)
        {
            ...
            if (marketPosition != MarketPosition.Flat)
            {
                ...
                currentTradeMfePts = 0.0;
                currentTradeMaePts = 0.0;
                Print("[v1.3-RC " + time.ToString("HH:mm:ss") + "] STRAT FRESH-ENTRY dir=" + currentEntryDir +
        ...
        protected override void OnBarUpdate()
        {
            ...
            {
                Print("ZigzagRunner v1.3-RC SAFETY: Position.Quantity=" + Position.Quantity +
        ...
        private void OnHardSlBarUpdate()
        {
            ...
            riskMgr.UpdateMaxPnlAndState(Position, c, currentEntryPrice, currentEntryDir);
            riskMgr.RouteStopForState(Position, c, currentEntryPrice, currentEntryDir, StopState_v13.Initial, StopState_v13.Tier1);
        }
        ...
        private void OnTrailTpBarUpdate()
        {
            ...
            riskMgr.UpdateMaxPnlAndState(Position, c, currentEntryPrice, currentEntryDir);
            riskMgr.RouteStopForState(Position, c, currentEntryPrice, currentEntryDir, StopState_v13.Tier2);
        }

        // ─── DynamicRiskManager_v13 callback ─────────────────────────────────
        ...
        private void RouteStopOrder(double stopPrice, int direction, double currentPrice)
        {
            ...
            if (valid)
            {
                ...
                Print("[v1.3-RC " + Time[0].ToString("HH:mm:ss") +
                      "] STRAT SetStopLoss sig=" + sig +
    ...
    // ═════════════════════════════════════════════════════════════════════
    // StopState_v13 + DynamicRiskManager_v13 (v1.3-RC variant)
    // ─────────────────────────────────────────────────────────────────────
    // CHANGE FROM v1.2: EvaluateStopState_v13() split into two operations so
    // the strategy can drive max-PnL tracking from one TF and stop
    // routing from another. Old EvaluateStopState_v13() retained as a
    // convenience wrapper.
    //
    ...
    //
    //   RouteStopForState(pos, price, params StopState_v13[] eligible)
    ...
    //
    //   EvaluateStopState_v13(pos, price)  ← back-compat (calls both)
    // ═════════════════════════════════════════════════════════════════════
    public enum StopState_v13 { Null, Initial, Tier1, Tier2 }

    public class DynamicRiskManager_v13
    {
        ...
        // State
        private StopState_v13 currentState   = StopState_v13.Null;
        private double maxUnrealizedPts  = 0.0;
        private double currentStopPrice  = 0.0;

        public DynamicRiskManager_v13(
        ...
        public StopState_v13 State          { get { return currentState; } }
        ...
        // sync now.
        public StopState_v13 UpdateMaxPnlAndState(Position position, double currentPrice, double entryPrice, int entryDir)
        {
        ...
        public void RouteStopForState(Position position, double currentPrice, double entryPrice, int entryDir, params StopState_v13[] eligibleStates)
        {
        ...
        {
            ResetState();
            currentState = StopState_v13.Initial;
            if (diagLogger != null)
        ...
        {
            currentState      = StopState_v13.Null;
        ...
        {
            StopState_v13 priorState = currentState;
            if (t2ActivationPts > 0.0 && maxPts >= t2ActivationPts)
                currentState = StopState_v13.Tier2;
            else if (t1ActivationPts > 0.0 && maxPts >= t1ActivationPts)
                currentState = StopState_v13.Tier1;
            else if (currentState == StopState_v13.Null)
                currentState = StopState_v13.Initial;
    ...
    // ═════════════════════════════════════════════════════════════════════
    // StagnationMonitor_v13 (carried forward from v1.2 — runs on Pivot TF in
    // v1.3-RC so MAX_NEGATIVE_BARS counts consecutive PIVOT-TF BARS, not
    // 1s ticks. Strategy recreates the instance per trade since
    // ResetState is private.)
    // ═════════════════════════════════════════════════════════════════════
    public class StagnationMonitor_v13
    {
        ...
        public StagnationMonitor_v13(int maxNegativeBars)
        {
