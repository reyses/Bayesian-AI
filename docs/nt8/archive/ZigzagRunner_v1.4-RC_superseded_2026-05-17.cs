// =============================================================================
// ZigzagRunner 1.4-RC -- 2026-04-26  (multi-TF + DAILY REGIME FILTER)
// =============================================================================
// CHANGELOG 1.4-RC (2026-04-26):
...
//     Tier1 trail floor.
//
//   * REFACTOR: DynamicRiskManager_v14.EvaluateStopState_v14() split into
...
//
//   * StagnationMonitor_v14: still per-trade-recreated, evaluated on the
...
//   2. Apply ZigzagRunner_v1.4-RC to ANY chart — primary TF is your
...
{
    public class ZigzagRunner_v14 : Strategy
    {
        // ── Settings ─────────────────────────────────────────────────────
        ...
        [NinjaScriptProperty]
        [Display(Name = "Enable Regime Filter", Description = "If true, skip trading on sessions where the prior-5-day mean daily range exceeds the threshold below. Filter evaluated once per session at start of first Pivot TF bar. Existing positions are still managed; only new entries are gated.", Order = 1, GroupName = "RegimeFilter")]
        public bool EnableRegimeFilter { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Max Mean Range 5d (points)", Description = "Threshold for the 5-day mean daily range filter. If mean(high-low) over last 5 daily bars > this value, skip the session. Default 350pt. Lower = stricter (more skipped days). Empirical median split at 351pt; bottom-quartile threshold ~250pt.", Order = 2, GroupName = "RegimeFilter")]
        public double MaxMeanRange5dPts { get; set; }

        // ── Direction modifier (v1.4-RC, added 2026-04-26) ───────────────
        ...
        // ── Version ──────────────────────────────────────────────────────
        private const string VERSION = "1.4-RC";
        ...
        private DynamicRiskManager_v14 riskMgr;
        private StagnationMonitor_v14 stagnationMon;
        ...
        private double lastEvalMeanRange5d  = 0.0;
        ...
        protected override void OnStateChange()
        {
            ...
            {
                Name                             = "ZigzagRunner_v1.4-RC";
                Description                      = "Multi-TF zigzag pivots + 3-TF risk machinery (Pivot/SL/Trail) + StagnationMonitor_v14 + CSV ledger. v" + VERSION;
            ...
            else if (State == State.Configure)
            {
                ...
                riskMgr = new DynamicRiskManager_v14(
                    HardStopLossPoints,
                ...
                // riskMgr.SetDiagLogger(s => Print("[v1.4-RC " + Time[0].ToString("HH:mm:ss") + "] " + s));
                stagnationMon = new StagnationMonitor_v14(MaxNegativeBars);
                EnsureCsvHeader();
        ...
        private void EnsureCsvHeader()
        {
            ...
            {
                Print("ZigzagRunner v1.4-RC CSV init error: " + ex.Message);
            }
        }
                ...
                if (!File.Exists(RegimeLogPath))
                {
                    lock (regimeLogLock)
                    {
                        File.WriteAllText(RegimeLogPath,
                            "log_time_utc,session_date,mean_range_5d,threshold_pts,trade_today,enable_filter" + Environment.NewLine);
                    }
                }
            }
            catch (Exception ex)
            {
                Print("ZigzagRunner v1.4-RC regime-log init error: " + ex.Message);
            }
        }

        private void AppendRegimeLog(DateTime sessionDate, double meanRange5d, bool tradeToday)
        {
            if (string.IsNullOrWhiteSpace(RegimeLogPath)) return;
            try
            {
                string row = string.Join(",", new string[] {
                    ...
                    meanRange5d.ToString("F4", CultureInfo.InvariantCulture),
                    MaxMeanRange5dPts.ToString("F4", CultureInfo.InvariantCulture),
            ...
            {
                Print("ZigzagRunner v1.4-RC regime-log append error: " + ex.Message);
            }
        }

        // Compute mean(high-low) over the prior 5 daily bars (BarsArray[BIP_DAILY]).
        // Returns NaN if fewer than 5 daily bars are available.
        private double ComputeMeanRange5d()
        {
            if (CurrentBars[BIP_DAILY] < 5) return double.NaN;
            double sum = 0.0;
            for (int i = 0; i < 5; i++)
        ...
        // Decides whether to trade today based on the 5-day mean daily range.
        private void EvaluateRegimeFilter(DateTime sessionDate)
        {
            ...
            double meanRange = ComputeMeanRange5d();
            if (double.IsNaN(meanRange))
            {
                // Not enough daily history yet (warmup) — default to trading.
                tradeAllowedToday = true;
                lastEvalMeanRange5d = 0.0;
                return;
            }
            lastEvalMeanRange5d = meanRange;
            tradeAllowedToday   = meanRange <= MaxMeanRange5dPts;
            string mode = RideWithTrend ? "WITH-trend" : "COUNTER-trend";
            Print("[v1.4-RC " + sessionDate.ToString("yyyy-MM-dd") +
                  "] RegimeFilter: meanRange5d=" + meanRange.ToString("F2") +
                  " threshold=" + MaxMeanRange5dPts.ToString("F2") +
                  " -> tradeToday=" + tradeAllowedToday +
                  " mode=" + mode);
            AppendRegimeLog(sessionDate, meanRange, tradeAllowedToday);
        }
        ...
        private void AppendTradeCsv(DateTime exitTime, string exitReason, double exitPrice, int qty)
        {
            ...
            {
                Print("ZigzagRunner v1.4-RC CSV append error: " + ex.Message);
            }
        }
                                                  ...
                                                  DateTime time)
        {
            ...
            if (marketPosition != MarketPosition.Flat)
            {
                ...
                currentTradeMfePts = 0.0;
                currentTradeMaePts = 0.0;
                Print("[v1.4-RC " + time.ToString("HH:mm:ss") + "] STRAT FRESH-ENTRY dir=" + currentEntryDir +
        ...
        protected override void OnBarUpdate()
        {
            ...
            {
                Print("ZigzagRunner v1.4-RC SAFETY: Position.Quantity=" + Position.Quantity +
        ...
        private void OnHardSlBarUpdate()
        {
            ...
            riskMgr.UpdateMaxPnlAndState(Position, c, currentEntryPrice, currentEntryDir);
            riskMgr.RouteStopForState(Position, c, currentEntryPrice, currentEntryDir, StopState_v14.Initial, StopState_v14.Tier1);
        }
        ...
        private void OnTrailTpBarUpdate()
        {
            ...
            riskMgr.UpdateMaxPnlAndState(Position, c, currentEntryPrice, currentEntryDir);
            riskMgr.RouteStopForState(Position, c, currentEntryPrice, currentEntryDir, StopState_v14.Tier2);
        }

        // ─── DynamicRiskManager_v14 callback ─────────────────────────────────
        ...
        private void RouteStopOrder(double stopPrice, int direction, double currentPrice)
        {
            ...
            {
                Print("[v1.4-RC " + Time[0].ToString("HH:mm:ss") +
    ...
    // ═════════════════════════════════════════════════════════════════════
    // StopState_v14 + DynamicRiskManager_v14 (v1.4-RC variant)
    // ─────────────────────────────────────────────────────────────────────
    // CHANGE FROM v1.2: EvaluateStopState_v14() split into two operations so
    // the strategy can drive max-PnL tracking from one TF and stop
    // routing from another. Old EvaluateStopState_v14() retained as a
    // convenience wrapper.
    //
    ...
    //
    //   RouteStopForState(pos, price, params StopState_v14[] eligible)
    ...
    //
    //   EvaluateStopState_v14(pos, price)  ← back-compat (calls both)
    // ═════════════════════════════════════════════════════════════════════
    public enum StopState_v14 { Null, Initial, Tier1, Tier2 }

    public class DynamicRiskManager_v14
    {
        ...
        // State
        private StopState_v14 currentState   = StopState_v14.Null;
        private double maxUnrealizedPts  = 0.0;
        private double currentStopPrice  = 0.0;

        public DynamicRiskManager_v14(
        ...
        public StopState_v14 State          { get { return currentState; } }
        ...
        // sync now.
        public StopState_v14 UpdateMaxPnlAndState(Position position, double currentPrice, double entryPrice, int entryDir)
        {
        ...
        public void RouteStopForState(Position position, double currentPrice, double entryPrice, int entryDir, params StopState_v14[] eligibleStates)
        {
        ...
        {
            ResetState();
            currentState = StopState_v14.Initial;
            if (diagLogger != null)
        ...
        {
            currentState      = StopState_v14.Null;
        ...
        {
            StopState_v14 priorState = currentState;
            if (t2ActivationPts > 0.0 && maxPts >= t2ActivationPts)
                currentState = StopState_v14.Tier2;
            else if (t1ActivationPts > 0.0 && maxPts >= t1ActivationPts)
                currentState = StopState_v14.Tier1;
            else if (currentState == StopState_v14.Null)
                currentState = StopState_v14.Initial;
    ...
    // ═════════════════════════════════════════════════════════════════════
    // StagnationMonitor_v14 (carried forward from v1.2 — runs on Pivot TF in
    // v1.4-RC so MAX_NEGATIVE_BARS counts consecutive PIVOT-TF BARS, not
    // 1s ticks. Strategy recreates the instance per trade since
    // ResetState is private.)
    // ═════════════════════════════════════════════════════════════════════
    public class StagnationMonitor_v14
    {
        ...
        public StagnationMonitor_v14(int maxNegativeBars)
        {
