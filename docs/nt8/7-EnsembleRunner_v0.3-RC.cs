...
{
    public class EnsembleRunner_v03 : Strategy
    {
        private const string VERSION = "0.3.0-RC";
        ...
        protected override void OnBarUpdate()
        {
            ...
            // STOP-RACE NOTE (v0.3, P2c): this poll runs on EVERY closed 5s bar, whereas the
            //   R-trigger reversal exit is only evaluated once per completed MINUTE (in
            //   RunDecision). So when the stop is ON, any adverse move that reaches the stop
            //   inside a minute fires the stop BEFORE the minute's R-trigger check runs -- this
            //   is one of the reasons v0.2 logged zero RTriggerReversal exits (the other, and
            //   larger, was the cold-warmup min_rev inflation now fixed in RollSession). The
            //   stop stays default OFF in SIM (EnableCatastrophicStop=false); do NOT enable it
            //   in SIM if you want to observe the ride-only R-trigger behavior.
            // STOP-SLIP NOTE (P2-10): the poll already uses the finest primary resolution (the
            //   5s bar Low/High, so it catches the intrabar excursion), but FlattenPosition
            //   submits a MARKET order that fills at the NEXT 5s bar's open. Residual slip =
            //   the adverse travel from the trigger price to that next-bar fill (v0.2 backtest
            //   saw 10-230% over the point setting on fast bars). A true native ExitLongStopMarket
            //   resting order (bounded slip) is deferred to live -- P2-10.
