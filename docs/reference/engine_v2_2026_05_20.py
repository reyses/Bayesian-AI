...
class LiveEngineV2:
    ...
    async def _step7_trade(self):
        ...
        while not self._shutting_down:
            ...
            is_flip = (entry_sig is not None
                        and any(d.exit_reason for d in batch.position_decisions))
    ...
    def _write_l5_overlay(self, bar, skip, events):
        ...
        try:
            ...
            if pivots:
                zz_pivots = '|'.join(f'{int(pt)}:{px:.2f}'
                                      for (pt, px, _kind) in pivots)
            else:
                zz_pivots = ''
            lines = [
            ...
            if skip is not None:
                lines.append(f'SKIP|{int(bar["timestamp"])}|{bar["close"]}|'
            ...
            for ev in events:
                if 'ENTRY' in ev:
                    ...
                    side = (getattr(eng, '_this_bar_rtrig_dir', '') or
                            (prim.direction if prim else ''))
                    lines.append(f'TRADE|{int(bar["timestamp"])}|'
                                  f'{bar["close"]}|{side}|ENTRY')
                elif 'EXIT' in ev:
                    lines.append(f'TRADE|{int(bar["timestamp"])}|'
                                  f'{bar["close"]}||EXIT')
