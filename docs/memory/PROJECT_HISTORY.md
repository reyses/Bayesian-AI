# Project History — Chronological Arc
> A ≤60-line era map of the whole project. Each era points to the detailed `## Source:` sections below (the verbatim archive of the deleted topic files) and to `docs/daily/`. Condensed 2026-06-12; the load-on-start summary is `MEMORY.md`.

1. **Quantum / physics era** (pre-2026) — physics-metaphor "field" engine; later fully purged. NOT quantum physics. → `project_quantum_reconnect`, `project_physics_engine`, `feedback_metaphor_origins`.
2. **V1 SFE + 79D NN + NMP tiers** (early 2026) — statistical field engine, 79D features, Nightmare Protocol (`z_se>2 ∧ vr<1`, inverse exit, 10-bar approach buffer), a 9-gate cascade that became 9 ExNMP tiers, and a Bayesian "brain" (templates → K-means distill → regret → cascade). Brain deleted commit 23db222f; the K-means-on-multi-D step was the red flag (false-merged unrelated patterns). → `project_original_bayesian_brain_architecture`, `project_nn_v2_system`, `project_dissect_old_brain`.
3. **Lookahead reckoning #1** (2026-04-17) — the $740/day baseline exposed as PURE lookahead (`searchsorted` shifted by period + higher-TF future-data aggregation). Honest post-fix IS baseline = −$164/day; every tier at noise floor. FEATURES dir renamed to break accidental reuse. → `project_honest_baseline_2026_04_17`, `feedback_lookahead_audit`.
4. **Blended CNN stack** (Feb–May 2026) — CNN flip/hold/risk + killshot + blended tiers on 79D; phantom-spike "edge" on NT8 data debunked on clean Databento. Entire supervised stack later DELETED. → `project_tradecnn_baseline`, `feedback_phantom_spikes`, `feedback_cnn_fragility`.
5. **V2 schema migration** (2026-05) — 185D layered per-family features; V2-native iso pipeline (9 ExNMP tiers ported); 2D regime labels; MA-alignment direction win (70.5% acc on 20% of bars, deterministic). V2-only became a hard rule. → `project_v2_native_training`, `project_v2_iso_pipeline`, `project_v2_ma_alignment_directional`, `project_v2_features_eda_stack`.
6. **Regret-oracle arc** (2026-05-16→) — 6-layer architecture; binary direction classifier AUC 0.864 (but direction-alone is not a live strategy, info ceiling ~83%); the during-trade B-stack paradigm; B9 (remaining-amplitude, continuous sizing) and B10 (vol-regime sizer) promoted; R-trigger proven STRUCTURALLY OPTIMAL for binary exit as 7 overlay families were killed one by one. → `project_regret_six_layer_architecture`, `project_during_trade_b_stack`, `project_b_c_naming_convention`.
7. **L5 SIM-deploy build** (2026-05-19→20) — L5Decider as a thin-wrapper engine, `engine_v2 --engine-mode l5`, forward-pass-validated (decision-identical to `forward_pass_full_stack`); first SIM run's −$379 traced to the B9-horizon bug (`bars_held` is MINUTES, K horizons are 5s-bar units) — found + fixed. → `project_l5_sim_deploy_2026_05_19`, `project_parity_b9_horizon_2026_05_20`, `feedback_thin_wrapper_live_engine`.
8. **Lookahead reckoning #2 / causal reckoning** (2026-05-22) — the +$454/day hardened-leg baseline is HINDSIGHT (offline zigzag = sequential partition, zero whipsaw); causal streaming = −$100 to −$185/day (replay +$417 vs stream −$185 through the same harness). L5 stack a WASH on NT8 OOS. DO NOT deploy real money. All trade-outcome-suite findings provisional. → `docs/daily/2026-05-22.md`, `feedback_flat_pipeline_cross_param`.
9. **RL pivot** (2026-05-28) — supervised CNN/blended/nightmare stack DELETED (commit 4b658e2a); replaced by the PW-CRL RL engine (CNN+LSTM DQN, V-trace, hindsight-regret shadow queue, curriculum EXIT_NMP→ENTRY_NMP→YOLO). Mid-training, not deployed; left ~15 dangling importers. → `project_rl_pivot_2026_05_28`, `rl_whitepaper.md`.
10. **Regression-segments / regime cloud** (2026-06) — segment-regression corpus under the SEGMENT FIREWALL (label-side only); FISTA CV bug found + fixed (2026-06-08) with a parity guard; `research_A` architecture variant; CI pseudoreplication / effective-N discipline established. → `reference_fista_gpu_cv_step_bug`, `feedback_ci_pseudoreplication_effective_n`.
11. **λ-completion program** (2026-06-11→, ACTIVE) — the equity-burning drawdown = the integral of the MISSING λ term: NMP's `λ` was hardcoded 0.0 and its real `vr` gate was dropped from V2. Recover it: Z* recalibrated to 1.8481/0.4752 on `L3_1m_z_se_15`; λ̂/vr derivation layer verified (`research/nmp_state/`); Stage-0 forward pass pending. → `ROADMAP_LAMBDA_COMPLETION.md`, `docs/Active/NMP_V2_FEATURE_MAP.md`.
