...
- **ORGANIZE RESEARCH FOLDERS** (2026-06-22): each research effort = ONE `research/<topic>/` folder, code in SUBFOLDERS (`pipeline/`/`builders/`/`tools/`) + `reports/` + `README.md`; NEVER flat, NEVER mixed into shared `reports/findings/`; big/gitignored data stays at repo root (run from root). Set up from the START (retrofit = path refactor). Example: `research/fspace_cadence/`. See [[organize-research-folders]]; in CLAUDE.md + Gemini research_discipline skill v2.
