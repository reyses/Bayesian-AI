...
- **JOURNALS** (start AND end of EVERY session): update `docs/daily/YYYY-MM-DD.md` + one-line in `docs/daily/INDEX.md` (newest first) + `docs/reference/RESEARCH_JOURNAL.txt` + finding reports in the project `research/<topic>/reports/`. Session start: read INDEX.md (not full journals). Lost journals = lost days.
...
- **ORGANIZE RESEARCH FOLDERS** (2026-06-22): each research effort = ONE `research/<topic>/` folder, code in SUBFOLDERS (`pipeline/`/`builders/`/`tools/`) + `reports/` + `README.md`; NEVER flat, NEVER mixed into shared top-level `reports/`; big/gitignored data stays at repo root (run from root). Set up from the START (retrofit = path refactor). Example: `research/fspace_cadence/`. See [[organize-research-folders]]; in CLAUDE.md + Gemini research_discipline skill v2. Reports routing: research -> research/<topic>/reports/; training/baseline -> training/reports/; top-level reports/ = ONLY standalone reports tied to neither a research project nor a training run.
...
- **TURNS LIVE IN PATHS, NOT SNAPSHOTS (2026-07-17, docs 089-092)**: 46 static detectors scored on ±2-min label-turn detection — NONE beat the 0.43 density null (best real precision 0.31=TURN-CLIMAX; best coverage 0.30=RENKO firehose); a 409-dim F-space snapshot ALSO lost to 4 trivial context features (doc 089). Entry solved (P calibrated, +3.86pts@5m top decile) but entries land at phase 0.64 → oracle-exit ceiling 23% vs Moises 50-80% target → the ±1-2min turn detector is THE binding constraint and it is a SEQUENTIAL problem (Mamba lane). Keepers for state: TURN-HA (coverage), EXIT-KMDR (leads −0.2m), TURN-CLIMAX (purity), CTX-ER (chop), fire-freshness, 4-feat P_hold. Brackets/fixed exits: dead internally (091) + 3 external replications.
...
- **Thinks topic-at-a-time** — sequential questions only; configurable defaults over preemptive engineering ("we won't know if we don't try"). **Format: DISTRIBUTIONS + MODE FIRST, NOT averages** ("average does not make sense to me", 2026-06-16) — lead with the histogram/mode; mean only WITH its CI and never as the headline; bucket, don't average; honest-floor $-framing; always show CIs. See `report-distributions-and-mode`.
...
## 8. POINTERS
- [Worker delegation ladder](feedback-worker-delegation-ladder.md) — STANDING PROTOCOL (2026-07-16): Fable=spec+verdict only; Opus=self-contained builds; Sonnet=mechanical verifiable runs; Haiku=watchers. ALT: if Fable unavailable, OPUS takes the reasoner seat w/ same discipline (escalate FPS-core + program verdicts). Minimize Fable usage; verify artifacts before use.
...
- `reference-mamba-ssm-wsl-perf.md` — Mamba RL runtime facts (2026-07-05): mamba-ssm step() silent-grad TRAP, WSL wheel matrix, CUPTI dead, cudagraph-vs-carried-state crash, eager ~53-61 bars/s baselines.
...
- [Telescope nested-cadence (2026-07-17)](project-telescope-nested-cadence.md) - per-TF context constant until bar CLOSES; forming bar never a bar, only closed sub-bars; dojo full-run instrument; assert close_ts<=frame_ts
- [Memory FTS mirror + promote ritual (2026-07-18)](feedback-session-promote-ritual.md) — Hermes-style ADD-ONLY segments: `tools/memory_loop/` builds a STRICTLY-DERIVED SQLite+FTS5 mirror (`docs/memory/memory.db`, gitignored) of MEMORY/detail/INDEX/RESEARCH_JOURNAL; `build_memory_db.py` rebuild → `query_memory.py "<terms>"` retrieve → `memory_stats.py` recurring-corrections + stale-flags. DB never a source/write-target. Session-end: promote to markdown FIRST, then rebuild.
- **CUT OVERLAYS CLOSED AT FULL POWER (2026-07-18, doc 107)**: on top-decile entries NO cut policy beats never-bail at N=23,378 natural mix (stops -3.4..-6.9 t/ep; re-entry -4.5; path-veto zero OOS discrimination; doc-100 +17.7 was a 1:1 balance artifact). Losers cut themselves; the edge is ride-only. Bar for any future cut idea: beat never-bail on powered_cut_frontier.py.
- [North star: teacher-student pipeline (2026-07-18, Moises)](project-teacher-student-northstar.md) - genome-evolved specialized gemma/qwen branch TEACHES Mamba (soft P(EXIT) labels, KL distillation) -> Mamba trades at production speed; gate: genome must show held-out ride-edge BEFORE any LoRA; distillation replaces most RL cost
- [Northstar reference set (2026-07-18)](docs/northstar/README.md) - docs/northstar/ = README index + review + gate spec (FROZEN copy governs at research/dojo_forge/RIDE_EDGE_GATE_SPEC.md) + hermes loop + ollama findings + Mamba-in-the-Llama paper
