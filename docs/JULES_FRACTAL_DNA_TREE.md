# JULES TASK: Fractal DNA Tree — Hierarchical Snowflake Cluster Schema

## The Core Insight

Every leaf PatternEvent (15s signal) already carries its full ancestry in `parent_chain`:
```python
p.parent_chain = [
    {'z': 1.8, 'type': 'ROCHE_SNAP', 'tf': '5m',  'dmi_plus': 12, ...},   # 5m parent
    {'z': 2.1, 'type': 'ROCHE_SNAP', 'tf': '15m', 'dmi_plus': 15, ...},   # 15m grandparent
    {'z': 2.4, 'type': 'ROCHE_SNAP', 'tf': '1h',  'dmi_plus': 20, ...},   # 1h great-grandparent
]
```

But `extract_features()` **collapses this entire ancestry to 4 scalar values**:
```python
parent_z, parent_dmi_diff, root_is_roche, tf_alignment   # lossy!
```

Two patterns that look the same at the leaf but come from completely different 1h/15m contexts
get assigned to the same cluster. This is the fundamental limitation of the star schema.

**The snowflake (fractal DNA) approach:**
Instead of one flat cluster space, build a hierarchical tree where each level is a cluster
of patterns AT THAT TIMEFRAME. The path from root to leaf is the pattern's DNA — the most
precise fingerprint of what market structure generated this signal.

```
ROOT
├── LONG
│   ├── [1h cluster 3: ROCHE_SNAP, z≈2.1, hurst=0.62]
│   │   ├── [15m cluster 7: STRUCTURAL_DRIVE, z≈1.4]
│   │   │   ├── [5m cluster 2: APPROACH, z≈0.9]
│   │   │   │   └── [15s cluster 9: ROCHE_SNAP, z≈1.8]  ← LEAF
│   │   │   │       DNA = "L|1h:3|15m:7|5m:2|15s:9"
│   │   │   └── [5m cluster 4: NOISE, z≈0.4]
│   │   └── [15m cluster 8: NOISE]
│   └── [1h cluster 5: STRUCTURAL_DRIVE, ...]
└── SHORT
    ├── [1h cluster 1: ROCHE_SNAP, ...]
    └── ...
```

The **DNA string** `"L|1h:3|15m:7|5m:2|15s:9"` uniquely identifies:
- Direction: LONG
- 1h context: cluster 3 (specific macro regime)
- 15m context: cluster 7 (specific intermediate structure)
- 5m context: cluster 2 (specific setup)
- 15s leaf: cluster 9 (specific entry trigger)

Patterns with the same DNA are the MOST homogeneous group in the system.
Oracle stats at the leaf node are the most predictive because they condition on
the FULL market context path, not just the leaf features.

---

## Architecture: Two New Data Structures

### `TreeNode` dataclass (new file: `training/fractal_dna_tree.py`)
...
## New Class: `FractalDNATree` (in `training/fractal_dna_tree.py`)
...
class FractalDNATree:
    ...
    def fit(self, patterns: List[PatternEvent]) -> None:
        """
        ...
        """
        long_patterns  = [p for p in patterns if p.oracle_marker > 0]
        short_patterns = [p for p in patterns if p.oracle_marker < 0]
    ...
    def _build_branch(self, patterns: List[PatternEvent], direction: str) -> TreeNode:
    ...
    def _split_by_tf_level(self, parent_node: TreeNode, patterns: list,
        ...
        leaf_tf    = patterns[0].timeframe  # the actual TF of these PatternEvents
    ...
    def _extract_tf_features(self, p: PatternEvent, tf: str, is_leaf: bool) -> List[float]:
        """
        ...
        if is_leaf:
            ...
            return [
                abs(p.z_score),
                np.log1p(abs(p.velocity)),
                np.log1p(abs(p.momentum)),
    ...
    def match(self, p: PatternEvent) -> tuple:
        """
        ...
        """
        direction = 'LONG' if p.z_score <= 0 else 'SHORT'
...
## Integration with `training/orchestrator.py`

### A. Build the tree at startup (after loading pattern_library)

```python
# After loading pattern_library, build DNA tree from training patterns
dna_tree_path = os.path.join(self.checkpoint_dir, 'fractal_dna_tree.pkl')
if os.path.exists(dna_tree_path):
    with open(dna_tree_path, 'rb') as f:
        self.dna_tree = pickle.load(f)
    print(f"  Loaded DNA tree: {len(self.dna_tree._dna_index)} leaf DNA paths")
else:
    self.dna_tree = None
    print("  DNA tree not found — run --fresh to build")
```

### B. At Gate 1 (cluster matching), augment with DNA match

```python
# After finding best_tid via centroid_index (existing flat match):
pattern_dna = None
dna_node = None
if self.dna_tree is not None:
    pattern_dna, dna_node, dna_conf = self.dna_tree.match(best_candidate)
    # Use DNA node stats if more specific (deeper tree, more members)
    # Fallback to flat template stats if DNA node has < 15 members
    if dna_node and dna_node.member_count >= 15:
        _mean_mfe_override = dna_node.mean_mfe_ticks   # more specific TP estimate
        _mean_mae_override = dna_node.mean_mae_ticks   # more specific SL estimate
```

### C. Log DNA in oracle_trade_records and signal_log

```python
pending_oracle['pattern_dna'] = str(pattern_dna) if pattern_dna else ''
```

At exit, the oracle_trade_log.csv gets a `pattern_dna` column:
`"L|1h:3|15m:7|5m:2|15s:9"` — the full fractal fingerprint of every trade.

### D. Add DNA to `_dm_rec` (signal log)
...
## Build the Tree During `--fresh` (Phase 3)

After `fractal_clustering.py` produces templates (existing code unchanged), build the DNA tree:

```python
from training.fractal_dna_tree import FractalDNATree

dna_tree = FractalDNATree(n_clusters_per_level=5)
dna_tree.fit(all_patterns)   # all_patterns = the same list used for clustering

with open(os.path.join(ckpt_dir, 'fractal_dna_tree.pkl'), 'wb') as f:
    pickle.dump(dna_tree, f)

print(f"  DNA tree: {len(dna_tree._dna_index)} unique DNA paths")
print(f"  LONG paths: {sum(1 for k in dna_tree._dna_index if k.startswith('L'))} | "
      f"SHORT paths: {sum(1 for k in dna_tree._dna_index if k.startswith('S'))}")
```
...
- `PatternTemplate` — existing flat clusters unchanged; DNA tree is additive
- `FractalClusteringEngine` — unchanged; both systems coexist
- Phase 3 DOE optimization — still runs on flat templates
- `WaveRider`, `TimeframeBeliefNetwork` — unchanged
- Oracle/FN logs — just add `pattern_dna` column (new, backward compatible)

The DNA tree is a **new layer on top** of the existing flat clustering, not a replacement.
Existing `pattern_library.pkl` continues to work. The DNA tree provides:
1. More precise oracle stats (conditioned on full TF context)
2. The `pattern_dna` field in every trade record for post-run analysis
3. Foundation for future step: replace flat matching entirely with tree traversal

---

## Files to Create / Modify

1. **`training/fractal_dna_tree.py`** — New file: `TreeNode`, `PatternDNA`, `FractalDNATree` classes
2. **`training/orchestrator.py`** — Load tree at startup, call `dna_tree.match()` at Gate 1, log `pattern_dna`
3. **Phase 3 build location** (wherever `create_templates()` is called) — add `FractalDNATree.fit()` call and save checkpoint

---

## Report Addition

After the DECISION MATRIX SUMMARY, add:

```
TOP 10 DNA PATHS BY WIN RATE  (min 10 trades):
  DNA                               Trades  Win%   Avg PnL
  L|1h:3|15m:7|5m:2|15s:9              47   87%   $182.50
  L|1h:3|15m:7|5m:4|15s:2              31   84%   $156.00
  S|1h:1|15m:4|15s:7                   28   82%   $143.00
  ...
```

This tells you exactly which fractal DNA paths are worth trading.
