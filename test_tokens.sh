...
t_exit=llm.tokenize(b"EXIT", add_bos=False)
t_hold=llm.tokenize(b"HOLD", add_bos=False)
print("EXIT tokens:", [llm.detokenize([t]).decode(errors="ignore") for t in t_exit], t_exit)
print("HOLD tokens:", [llm.detokenize([t]).decode(errors="ignore") for t in t_hold], t_hold)
'
