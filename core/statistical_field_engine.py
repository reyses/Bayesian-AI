...
def _compute_swing_noise_numba(highs: np.ndarray, lows: np.ndarray, n: int, noise_window: int, tick_size: float):
...
class StatisticalFieldEngine:
    ...
    def batch_compute_states(self, day_data: pd.DataFrame, use_cuda: bool = True, params: dict = None) -> list:
        ...
        # Numba JIT: ~300x vs sliding window loop + allocate (0.017s -> 0.000s per day)
