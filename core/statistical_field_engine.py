...
def _compute_swing_noise_numba(highs: np.ndarray, lows: np.ndarray, noise_window: int, tick_size: float) -> np.ndarray:
    ...
    for i in prange(noise_window, n):
        # We process the segment backwards to track min/max running values efficiently

        # Max drawdown from running high (long-side noise)
        max_high = highs[i]
        max_dd = max_high - lows[i]

        # Max drawup from running low (short-side noise)
        min_low = lows[i]
        max_du = highs[i] - min_low

        for j in range(i - 1, i - noise_window - 1, -1):
            if highs[j] > max_high:
                max_high = highs[j]
            curr_dd = max_high - lows[j]
            if curr_dd > max_dd:
                max_dd = curr_dd

            if lows[j] < min_low:
                min_low = lows[j]
            curr_du = highs[j] - min_low
            if curr_du > max_du:
                max_du = curr_du
...
class StatisticalFieldEngine:
    ...
    def batch_compute_states(self, day_data: pd.DataFrame, use_cuda: bool = True, params: dict = None) -> list:
        ...
        # Numba JIT: ~250x vs np.maximum.accumulate array slicing (1.04s -> 0.004s)
