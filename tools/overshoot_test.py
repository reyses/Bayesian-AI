"""
Overshoot Test — NMP base + hold through mean for momentum overshoot.

Usage:
    python tools/overshoot_test.py                    # IS only
    python tools/overshoot_test.py --target oos       # OOS
"""
import os
import sys
import glob
...
def parse_args():
    import argparse
    p = argparse.ArgumentParser(description='Overshoot strategy test')
...
def main():
    ...
    print(f'OVERSHOOT TEST — NMP + Hold Through Mean')
    ...
    print(f'\n{"="*60}')
    print(f'OVERSHOOT RESULTS — {args.target.upper()}')
    print(f'{"="*60}')
    ...
    if all_trades > 0:
        ...
        # Reload trades from last engine run for exit stats
        # (we don't accumulate across days, but summary is enough)
