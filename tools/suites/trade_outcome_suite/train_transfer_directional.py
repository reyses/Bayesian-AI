...
def train():
    ...
    t_w = torch.tensor(w, dtype=torch.float32)
    ...
    v_w = torch.tensor(w_val, dtype=torch.float32)
    
    t_dense = torch.tensor(X_dense, dtype=torch.float32)
    v_dense = torch.tensor(X_dense_val, dtype=torch.float32)
    
    train_ds = TensorDataset(t_grid, t_tod, t_reg, t_dense, t_y, t_w)
    val_ds = TensorDataset(v_grid, v_tod, v_reg, v_dense, v_y, v_w)
    ...
    # TRANSFER LEARNING: Load the pre-trained Unweighted Model (Perfection)
    print("Loading Pre-Trained Unweighted Model (Perfection)...")
    ...
    # TRANSFER LEARNING: Very low learning rate to fine-tune without destroying pre-trained geometry
    optimizer = torch.optim.AdamW(model.parameters(), lr=1e-5, weight_decay=1e-3)
    
    epochs = 25
    best_auc = 0.0
    
    out_dir = 'checkpoints/transfer_directional'
    os.makedirs(out_dir, exist_ok=True)
    
    for ep in range(epochs):
        model.train()
        train_loss = 0.0
        for b_grid, b_tod, b_reg, b_dense, b_y, b_w in train_ld:
