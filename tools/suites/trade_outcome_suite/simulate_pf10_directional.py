...
def evaluate_thresholds():
    ...
    with torch.no_grad():
        for i in range(0, len(X_grid), batch_size):
            b_grid = v_grid[i:i+batch_size]
            b_tod = v_tod[i:i+batch_size]
            b_reg = v_reg[i:i+batch_size]
            b_dense = v_dense[i:i+batch_size]
            logits = model(b_grid, b_tod, b_reg, b_dense)
            b_probs = torch.sigmoid(logits).cpu().numpy().flatten()
            probs_list.append(b_probs)
    ...
    true_is_winner = (valid_pnl > 0).astype(int)
    scores = np.where(valid_dir == 1, probs, 1.0 - probs)
    try:
        auc = roc_auc_score(true_is_winner, scores)
        print(f"OOS AUC (Filter Logic): {auc:.3f}")
        
        # Raw directional AUC
        raw_true_dir = np.where(true_is_winner == 1, valid_dir, 1 - valid_dir)
        raw_auc = roc_auc_score(raw_true_dir, probs)
        print(f"OOS AUC (Directional): {raw_auc:.3f}")
    ...
    for thresh in [0.10, 0.20, 0.30, 0.40, 0.50, 0.60, 0.70, 0.80, 0.90, 0.95]:
        ...
        total_pnl_abs = gross_profit + gross_loss
        wr = (gross_profit / total_pnl_abs) if total_pnl_abs > 0 else 0.0
        
        print(f"Threshold > {thresh:.2f} | Trades: {n_trades:4d} | Net PnL: ${net:7,.0f} | Mag WR: {wr:.3f} | Profit Factor: {pf:5.2f}")

    print("\n--- Transfer Learning Directional Override (Stay or Flip) ---")
    ...
    gross_profit_override = override_pnl[override_pnl > 0].sum()
    gross_loss_override = abs(override_pnl[override_pnl <= 0].sum())
    total_abs_override = gross_profit_override + gross_loss_override
    wr_override = (gross_profit_override / total_abs_override) if total_abs_override > 0 else 0.0
    ...
    print(f"Transfer Override Mag WR: {wr_override:.3f}\n")

    print("--- 3-Option Logic (Continue, Flip, Skip) ---")
    ...
    for thresh in [0.55, 0.60, 0.65]:
        ...
        if n_total > 0:
            gross_prof_total = pnl_continue[pnl_continue > 0].sum() + pnl_flip[pnl_flip > 0].sum()
            gross_loss_total = abs(pnl_continue[pnl_continue <= 0].sum()) + abs(pnl_flip[pnl_flip <= 0].sum())
            tot_abs = gross_prof_total + gross_loss_total
            wr_total = (gross_prof_total / tot_abs) if tot_abs > 0 else 0.0
        else:
            wr_total = 0.0
        ...
        print(f"  COMBINED | Trades: {n_total:3d} | Net: ${net_total:6,.0f} | Mag WR: {wr_total:.3f}\n")
