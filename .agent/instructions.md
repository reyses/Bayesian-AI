...
1. **High-Level Discussion**: Present a high-level analysis of the problem, the mathematical/logical flaw, and the proposed approach (with risk assessment).
2. **Approval 1**: Wait for the user to approve the high-level approach.
3. **Implementation Plan**: Create a detailed Implementation Plan document (artifact) listing *every* file to be modified and the exact mechanical changes to be made.
4. **Approval 2**: Wait for the user to approve the Implementation Plan.
5. **Execute**: Only after explicit approval of the detailed plan, execute the changes.
...
When making images, docs, etc for user review (especially implementation docs or walkthroughs) AND we are in telegram mode, always send them to the user via Telegram directly.

## To-Do List Memory Rule
Keep a persistent to-do list in the memory folders (.agent/todos/) to park ideas or tasks for later. Maintain two groups of files: an indexed to-do list file (.agent/todos/index.md) and a directory containing detailed files for each individual to-do item (.agent/todos/items/).
