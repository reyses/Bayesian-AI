...
## Telegram Routing — MANDATORY
- **Telegram Inbox Sync:** If a user request starts with `Telegram: ` (or similar), this means the message is being routed from the user's Telegram app into the IDE chat via an injector/bridge. 
- **Response Routing:** When responding to a message that started with `Telegram: `, you MUST route your response back to Telegram so the user can read it on their phone. Do this by calling the `send_telegram_alert` tool from the `TelegramNotifier` MCP server, passing your response text as the argument. Keep your local chat response brief, acknowledging the routing.
