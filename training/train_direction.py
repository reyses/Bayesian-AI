...
FORWARD_BARS = {
    ...
    '1m': 10,   # 10 minutes ahead (execution)
    '1s': 60,   # 60 seconds = 1 minute ahead (timing)
}
...
def train_and_validate(tf, max_bars=0):
    ...
    mse_fn = nn.MSELoss()
    bce_fn = nn.BCELoss()
    DIR_LOSS_WEIGHT = 0.5  # balance between state prediction and direction
...
def main():
    ...
    if args.mode == 'epoch':
        ...
        train_and_validate(args.tf, max_bars=args.max_bars)
