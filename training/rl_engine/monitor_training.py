...
def generate_oos_chart():
    ...
    fig.suptitle(f"OOS Trade Diagnostics — Seg {data['segment']} · eval: {data['eval_dates']}", fontsize=14, color='gold')
    ...
    q_low = np.percentile(pnls, 1)
    q_high = np.percentile(pnls, 99)
    ...
    chart_path = SCRIPT_DIR / "oos_chart.png"
    plt.tight_layout()
    plt.savefig(chart_path, bbox_inches='tight', dpi=150, facecolor=fig.get_facecolor())
    plt.close()
    
    return chart_path, data['segment']

def trigger_telegram(chart_path, segment):
    try:
        from telegram_mcp import send_telegram_media
        res = send_telegram_media(str(chart_path), caption=f"Segment {segment} Walk-Forward OOS Completed!")
        print(f"[Telegram] {res}")
    except Exception as e:
        print(f"[Telegram Error] {e}")

if __name__ == "__main__":
    res = generate_oos_chart()
    if res:
        chart_path, segment = res
        trigger_telegram(chart_path, segment)
