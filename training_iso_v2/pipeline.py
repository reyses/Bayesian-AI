...
Usage:
    python -m training_iso_v2.pipeline                  # full pipeline (cached calibrations)
    python -m training_iso_v2.pipeline --fresh          # rebuild all calibrations
    python -m training_iso_v2.pipeline --from 3         # skip calibration, just run iso + analysis
    python -m training_iso_v2.pipeline --is-only        # phases 1-4
    python -m training_iso_v2.pipeline --oos-only       # phases 1-2 + 5-6
...
def main():
    ...
    if in_range(2):
        run_phase(
            ...
            [py, '-m', 'training_iso_v2.calibrate_tiers',
    ...
    iso_base = [py, '-m', 'training_iso_v2.run_iso',
