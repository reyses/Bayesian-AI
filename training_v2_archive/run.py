...
def _run_nmp_fast(target: str, equity: float = None):
    ...
    if all_trades:
        import pickle
        os.makedirs('training_v2/output/trades', exist_ok=True)
        # Determine label from target
        label = target if target in ('is', 'oos', 'all') else 'custom'
        trade_path = f'training_v2/output/trades/nmp_{label}.pkl'
        ...
        for t in all_trades:
            ...
            flat.append(row)
        csv_path = f'training_v2/output/trades/nmp_{label}.csv'
...
def _run_regret():
    ...
    # Load trades
    with open('training_v2/output/trades/nmp_is.pkl', 'rb') as f:
    ...
    # Save
    os.makedirs('training_v2/output/nn', exist_ok=True)
    regret_df.to_csv('training_v2/output/nn/regret_analysis.csv', index=False)
    print(f'\n  Saved: training_v2/output/nn/regret_analysis.csv')
...
def _run_gated(target: str):
    ...
    tree_path = 'training_v2/output/nn/strategy_tree.pkl'
    ...
    # Save report
    os.makedirs('training_v2/output/nn', exist_ok=True)
    report_path = f'training_v2/output/nn/gated_{target}_report.txt'
    ...
    csv_path = f'training_v2/output/nn/gated_{target}_daily.csv'
    pd.DataFrame(all_results).to_csv(csv_path, index=False)
    print(f'CSV saved: {csv_path}')

    # Save memory
    memory.save(f'training_v2/output/nn/memory_{target}.pkl')
    ...
    playbook_path = f'training_v2/output/nn/playbook_{target}.txt'
    ...
    try:
        ...
        plt.tight_layout()
        plot_path = f'training_v2/output/nn/gated_{target}_equity.png'
...
def _run_ai(target: str):
    ...
    os.makedirs('training_v2/output/nn', exist_ok=True)
    if all_trades:
        trade_path = f'training_v2/output/nn/ai_{target}_trades.pkl'
        with open(trade_path, 'wb') as f:
            pickle.dump(all_trades, f)
        print(f'Trade log saved: {trade_path} ({len(all_trades)} trades)')

    # Save report
    report_path = f'training_v2/output/nn/ai_{target}_report.txt'
    with open(report_path, 'w', encoding='utf-8') as f:
        n_days = len(all_results)
        total_pnl = sum(r['pnl'] for r in all_results)
        winning = sum(1 for r in all_results if r['pnl'] > 0)
    ...
    csv_path = f'training_v2/output/nn/ai_{target}_daily.csv'
    pd.DataFrame(all_results).to_csv(csv_path, index=False)
    print(f'CSV saved: {csv_path}')
...
def _run_ai_regret(target: str):
    ...
    # Save
    regret_path = f'training_v2/output/nn/ai_{target}_regret.csv'
    regret_df.to_csv(regret_path, index=False)
    branch_path = f'training_v2/output/nn/ai_{target}_regret_by_branch.csv'
...
def _run_full_pipeline():
    ...
    sys.argv = ['report.py',
                '--is-csv', 'training_v2/output/nn/ai_is_daily.csv',
                '--oos-csv', 'training_v2/output/nn/ai_oos_daily.csv']
    report_main()
    ...
    print(f'{"="*60}')
    print(f'  NMP regret:   training_v2/output/nn/regret_analysis.csv')
    print(f'  AI IS trades: training_v2/output/nn/ai_is_trades.pkl')
    print(f'  AI IS regret: training_v2/output/nn/ai_is_regret.csv')
    print(f'  AI OOS regret:training_v2/output/nn/ai_oos_regret.csv')
    print(f'  Report:       training_v2/output/nn/system_report.txt')
    print(f'  Book:         training_v2/output/nn/strategy_book.txt')
...
def _run_ai_with_book(target: str, book_pkl_path: str, label: str):
    ...
    # Save
    os.makedirs('training_v2/output/books', exist_ok=True)
    if all_trades:
        with open(f'training_v2/output/books/{label}_{target}_trades.pkl', 'wb') as f:
            pickle.dump(all_trades, f)

    csv_path = f'training_v2/output/books/{label}_{target}_daily.csv'
    pd.DataFrame(all_results).to_csv(csv_path, index=False)
...
def _run_blended_nmp(target: str, use_cnn: bool = True, verbose: bool = False):
    ...
    if all_trades:
        ...
        label = target if target in ('is', 'oos', 'all') else 'custom'
        # Save as blended
        trade_path = f'training_v2/output/trades/blended_{label}.pkl'
        with open(trade_path, 'wb') as f:
            pickle.dump(all_trades, f)
        # Also save to nmp path (pipeline downstream reads nmp_is.pkl)
        nmp_path = f'training_v2/output/trades/nmp_{label}.pkl'
        ...
        for t in all_trades:
            ...
            flat.append(row)
        csv_path = f'training_v2/output/trades/blended_{label}.csv'
        pd.DataFrame(flat).to_csv(csv_path, index=False)
...
def _run_bayesian_pipeline():
    ...
    os.makedirs('training_v2/output/trades', exist_ok=True)
    with open('training_v2/output/trades/corrected_is.pkl', 'wb') as f:
    ...
    book = VersionedBook.from_nmp_book('training_v2/output/nn/strategy_book.pkl')
    ...
    h0_is = pd.read_csv('training_v2/output/books/h0_is_daily.csv')
    h1_is = pd.read_csv('training_v2/output/books/h1_is_daily.csv')

    h0_is_stats = compute_stats(h0_is, 'H0-IS')
    h1_is_stats = compute_stats(h1_is, 'H1-IS')

    # Try OOS
    h0_oos_path = 'training_v2/output/books/h0_oos_daily.csv'
    h1_oos_path = 'training_v2/output/books/h1_oos_daily.csv'
    h0_oos_stats = {}
    h1_oos_stats = {}
    ...
    print(f'  Evolution:   training_v2/output/books/evolution.csv')
    print(f'  H0 IS:       training_v2/output/books/h0_is_daily.csv')
    print(f'  H1 IS:       training_v2/output/books/h1_is_daily.csv')
...
def _run_blended_pipeline(from_phase=None, to_phase=None):
    ...
    if _should_run('1'):
        ...
        print(f'  Done in {_time.perf_counter()-t0:.0f}s')
        _log_phase('1. Physics IS', trades_path='training_v2/output/trades/blended_is.pkl')
        ...
        print(f'  Done in {_time.perf_counter()-t0:.0f}s')
        _log_phase('1b. Physics OOS', csv_path='training_v2/output/blended/physics_oos_daily.csv')
        ...
        if nt8_feat_files:
            ...
            print(f'  Done in {_time.perf_counter()-t0:.0f}s')
            _log_phase('1c. Physics NT8', csv_path='training_v2/output/blended/physics_oos_nt8_daily.csv')
    ...
    if _should_run('3'):
        ...
        labels_path = 'training_v2/output/nn/tier_labels.pkl'
    ...
    if _should_run('6'):
        ...
        print(f'  Done in {_time.perf_counter()-t0:.0f}s')
        _log_phase('6. +CNN IS', trades_path='training_v2/output/trades/blended_is.pkl')
    ...
    if _should_run('7'):
        ...
        _log_phase('7. IS (final)', csv_path='training_v2/output/blended/is_daily.csv')
        _log_phase('7. OOS (final)', csv_path='training_v2/output/blended/oos_daily.csv')
        ...
        if nt8_feat_files:
            ...
            print(f'  Done in {_time.perf_counter()-t0:.0f}s')
            _log_phase('7. OOS-NT8', csv_path='training_v2/output/blended/oos_nt8_daily.csv')
        else:
            print(f'\n  OOS-NT8: No features in {NT8_FEATURES_5S}/ — skipping')

    # Hourly OOS report — only when Phase 7 ran (otherwise stale)
    if _should_run('7') and os.path.exists('training_v2/output/blended/oos_trades.pkl'):
        try:
            from tools.hourly_oos_report import hourly_report
            hourly_report('training_v2/output/blended/oos_trades.pkl')
        except Exception as e:
            print(f'  Hourly report failed: {e}')

    # Physics vs CNN comparison — only when BOTH ran in this pipeline
    physics_oos = 'training_v2/output/blended/physics_oos_daily.csv'
    cnn_oos = 'training_v2/output/blended/oos_daily.csv'
    ...
    oos_nt8_path = 'training_v2/output/blended/oos_nt8_daily.csv'
    ...
    oos_path = 'training_v2/output/blended/oos_daily.csv'
    ...
    if ran_physics:
        report_rows.append(('Physics IS',  'training_v2/output/trades/blended_is.pkl', True))
        report_rows.append(('Physics OOS', 'training_v2/output/blended/physics_oos_daily.csv', True))
        report_rows.append(('Physics NT8', 'training_v2/output/blended/physics_oos_nt8_daily.csv', True))
    if ran_cnn:
        report_rows.append(('IS',          'training_v2/output/blended/is_daily.csv', True))
        report_rows.append(('OOS',         'training_v2/output/blended/oos_daily.csv', True))
        report_rows.append(('OOS-NT8',     'training_v2/output/blended/oos_nt8_daily.csv', True))
    # If only physics ran, show CNN rows as cached for reference
    if ran_physics and not ran_cnn:
        for lbl, p in [('IS (cached)',     'training_v2/output/blended/is_daily.csv'),
                       ('OOS (cached)',    'training_v2/output/blended/oos_daily.csv'),
                       ('OOS-NT8 (cached)','training_v2/output/blended/oos_nt8_daily.csv')]:
            if os.path.exists(p):
...
BASELINE_FILE = 'training_v2/output/baseline_best.json'
...
def _run_blended_forward_physics_only(target: str):
    ...
    # Save
    os.makedirs('training_v2/output/blended', exist_ok=True)
    csv_path = f'training_v2/output/blended/physics_{target}_daily.csv'
    pd.DataFrame(all_results).to_csv(csv_path, index=False)
    print(f'Saved: {csv_path}')
...
def _check_new_baseline(oos_csv_path):
    ...
    # Load IS results
    is_path = 'training_v2/output/blended/is_daily.csv'
    df_is = pd.read_csv(is_path) if os.path.exists(is_path) else pd.DataFrame()

    # Load trade breakdowns
    tier_report = ''
    for label, pkl_path in [('IS', 'training_v2/output/blended/is_trades.pkl'),
                             ('OOS', 'training_v2/output/blended/oos_trades.pkl')]:
...
def _run_blended_forward_on_files(feat_files: list, label: str,
    ...
    os.makedirs('training_v2/output/blended', exist_ok=True)
    if all_trades:
        trade_path = f'training_v2/output/blended/{label}_trades.pkl'
        with open(trade_path, 'wb') as f:
            pickle.dump(all_trades, f)

    csv_path = f'training_v2/output/blended/{label}_daily.csv'
    pd.DataFrame(all_results).to_csv(csv_path, index=False)
    print(f'Saved: {csv_path}')
