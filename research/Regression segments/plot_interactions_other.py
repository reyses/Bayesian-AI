...
def main():
    ...
    for f_path in files:
        ...
        for seg in segments:
            ...
            # Focus on RECOVERED/MESSY tiers (3 through 8)
            if 3 <= tier <= 8:
    ...
    print(f"[INFO] Processed {total_segments} messy/recovered segments (Tiers 3-8) with interaction terms.")
    ...
    print("\n[TOP INTERACTIONS FOR TIERS 3-8]")
    ...
    if not unique_active_features:
        print("[INFO] No interactions found.")
        return
    ...
    sns.heatmap(sub_matrix, xticklabels=labels, yticklabels=labels, cmap="BuPu", annot=False)
    plt.title("Aggregated Polynomial Interaction Matrix (Messy Tiers 3-8)", fontsize=16)
    plt.xticks(rotation=90, fontsize=8)
    plt.yticks(rotation=0, fontsize=8)
    plt.tight_layout()
    plt.savefig('artifacts/combined_segment_interactions_tiers_3_8.png', dpi=150)
    print("\n[INFO] Saved heatmap to artifacts/combined_segment_interactions_tiers_3_8.png")
