...
def run_logistic_regression():
    ...
    if not os.path.exists(events_path):
        print(f"[SEASON-12_DayOfWeek] No events.parquet found. Run level 1 deepdive first.")
        return
        
    df = pd.read_parquet(events_path)
    if len(df) < 10:
        print(f"[SEASON-12_DayOfWeek] events.parquet has too few events (N={len(df)}).")
        return
        
    print(f"[SEASON-12_DayOfWeek] Loaded {len(df)} events for Logistic Regression.")
    ...
    report_lines = []
    report_lines.append(f"# Logistic Regression DOE: SEASON-12_DayOfWeek")
    ...
    print(f"[SEASON-12_DayOfWeek] Complete. Report written to {report_path}")
