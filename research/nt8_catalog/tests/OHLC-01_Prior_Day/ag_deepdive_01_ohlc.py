...
def process_day(args):
    ...
    # Setup 1 (Bearish Bounce off Resistance): If Open < PDH, scan forward. If price >= PDH, trigger 'bearish_bounce'.
    if open_price < pdh:
        for i, p in enumerate(prices):
            if p >= pdh:
                
                
                events.append({'setup': 1, 'mode': 'bearish_bounce', 'event_idx': i})
                break
                
    # Setup 2 (Bullish Bounce off Support): If Open > PDL, scan forward. If price <= PDL, trigger 'bullish_bounce'.
    if open_price > pdl:
        for i, p in enumerate(prices):
            if p <= pdl:
                events.append({'setup': 2, 'mode': 'bullish_bounce', 'event_idx': i})
                break
                
    # Setup 3 (PDC Gap Fill): If abs(Open - PDC) > 2.5 (10 ticks).
    if abs(open_price - pdc) > 2.5:
        if open_price < pdc:
            for i, p in enumerate(prices):
                if p >= pdc:
                    events.append({'setup': 3, 'mode': 'bullish_bounce', 'event_idx': i})
                    break
        else: # open_price > pdc
            for i, p in enumerate(prices):
                if p <= pdc:
                    events.append({'setup': 3, 'mode': 'bearish_bounce', 'event_idx': i})
                    break
...
if __name__ == '__main__':
    ...
    print(f"[OHLC Deep Dive] Computing Prior Day OHLC for {len(days)} days...")
    daily_ohlc = {}
    ...
    print("[OHLC Deep Dive] Evaluating Setups...")
    tasks = []
    ...
    print(f"[OHLC Deep Dive] Extracted {len(df)} triggered events.")
    ...
    report_lines = []
    report_lines.append("# Document ID: AG-DOC-OHLC-01 (LOGISTIC REGRESSION VERIFIED)")
    report_lines.append("**Title:** Deep Dive #1: Prior-day OHLC Levels")
    ...
    for year in ['2024', '2025']:
        ...
        for setup in [1, 2, 3]:
            ...
            n = len(df_sub)
            desc = "PDH Bearish Bounce" if setup == 1 else "PDL Bullish Bounce" if setup == 2 else "PDC Gap Fill Bounce"
    ...
    plot_path = os.path.join(assets_dir, 'DOC-OHLC-01_distributions.png')
    fig, axes = plt.subplots(1, 3, figsize=(18, 5))
    fig.suptitle('DOC-OHLC-01: Realizable Prior-day OHLC Setups (Both Years)', fontsize=16)
    ...
    report_lines.append(f"![Distribution Plot](./DOC-OHLC-01_distributions.png)")
    
    report_path = os.path.join(os.path.dirname(__file__), 'DOC_OHLC_01.md')
    with open(report_path, 'w') as f:
        f.write("\n".join(report_lines))
        
    print(f"[OHLC Deep Dive] Complete. Report written to {report_path}")
