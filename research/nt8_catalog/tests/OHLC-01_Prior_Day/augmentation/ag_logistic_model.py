...
def run_logistic_regression():
    ...
    if not os.path.exists(events_path):
        print(f"[OHLC-01_Prior_Day] No events.parquet found. Run level 1 deepdive first.")
        return
        
    df = pd.read_parquet(events_path)
    if len(df) < 10:
        print(f"[OHLC-01_Prior_Day] events.parquet has too few events (N={len(df)}).")
        return
        
    print(f"[OHLC-01_Prior_Day] Loaded {len(df)} events for Logistic Regression.")
    ...
    report_lines = []
    report_lines.append(f"# Logistic Regression DOE: OHLC-01_Prior_Day")
    ...
    print(f"[OHLC-01_Prior_Day] Complete. Report written to {report_path}")
