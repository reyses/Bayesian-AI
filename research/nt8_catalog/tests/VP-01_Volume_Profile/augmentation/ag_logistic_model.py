...
def run_logistic_regression():
    ...
    if not os.path.exists(events_path):
        print(f"[VP-01_Volume_Profile] No events.parquet found. Run level 1 deepdive first.")
        return
        
    df = pd.read_parquet(events_path)
    if len(df) < 10:
        print(f"[VP-01_Volume_Profile] events.parquet has too few events (N={len(df)}).")
        return
        
    print(f"[VP-01_Volume_Profile] Loaded {len(df)} events for Logistic Regression.")
    ...
    report_lines = []
    report_lines.append(f"# Logistic Regression DOE: VP-01_Volume_Profile")
    ...
    print(f"[VP-01_Volume_Profile] Complete. Report written to {report_path}")
