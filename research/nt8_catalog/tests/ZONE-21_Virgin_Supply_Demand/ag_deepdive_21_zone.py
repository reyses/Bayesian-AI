...
def process_day(day):
    ...
    df['vol_sma20'] = df['volume'].rolling(20).mean().bfill()
    ...
    atr = df_day['atr14'].values
    vol_sma = df_day['vol_sma20'].values
    sigmas = df_day['sigma'].values
    
    events_found = []
    
    # Store active virgin zones
    # [(setup, zone_high, zone_low)]
    active_zones = []
    
    # Wait at least 14 bars for ATR to stabilize
    for i in range(14, len(cl) - 60):
        # 1. Detect formation of a new zone
        # We need 3 tight candles followed by 1 explosion
        if i >= 3:
            tight_1 = (hi[i-3] - lo[i-3]) < atr[i-3] * 0.8
            tight_2 = (hi[i-2] - lo[i-2]) < atr[i-2] * 0.8
            tight_3 = (hi[i-1] - lo[i-1]) < atr[i-1] * 0.8
            
            if tight_1 and tight_2 and tight_3:
                # Explosion bar (i)
                bar_size = abs(cl[i] - op[i])
                is_explosion = bar_size > atr[i] * 1.5 and vo[i] > vol_sma[i] * 1.5
                
                if is_explosion:
                    z_high = max(hi[i-3], hi[i-2], hi[i-1])
                    z_low = min(lo[i-3], lo[i-2], lo[i-1])
                    if cl[i] > op[i]: # Bullish explosion -> Demand zone
                        active_zones.append((1, z_high, z_low))
                    else: # Bearish explosion -> Supply zone
                        active_zones.append((2, z_high, z_low))
        
        # 2. Check if current price touches any active virgin zone
        setup = 0
        mode = 'none'
        event_idx = -1
        
        hit_zone_idx = -1
        for z_idx, (z_setup, z_high, z_low) in enumerate(active_zones):
            if z_setup == 1 and lo[i] <= z_high: # Price fell into demand zone
                setup = 1
                mode = 'bullish_retest'
                event_idx = i
                hit_zone_idx = z_idx
                break
            elif z_setup == 2 and hi[i] >= z_low: # Price rallied into supply zone
                setup = 2
                mode = 'bearish_retest'
                event_idx = i
                hit_zone_idx = z_idx
        ...
        if hit_zone_idx != -1:
            # Remove the zone since it's no longer virgin
            active_zones.pop(hit_zone_idx)
        ...
        'duration_bars': _exit_idx if '_exit_idx' in locals() else -1,
                        'depth': (lambda l: next((abs(float(l[k])) for k in ['div', 'adx_val', 'z', 'z_val', 'z_score', 'distance', 'gap'] if k in l and l[k] is not None), abs(l.get('p0',0) - l.get('open_price',0)) if 'p0' in l and 'open_price' in l else 0.0))(locals()),
                    'mae': mae,
...
if __name__ == '__main__':
    ...
    print(f"[ZONE-21 Deep Dive] Evaluating Setups across {len(days)} days...")
    tasks = days
    ...
    if len(all_events) == 0:
        print("[ZONE-21 Deep Dive] No events found.")
    ...
    print(f"[ZONE-21 Deep Dive] Extracted {len(df)} triggered events.")
    ...
    report_lines = []
    report_lines.append("# Document ID: AG-DOC-ZONE-21")
    report_lines.append("**Title:** Deep Dive #21: Virgin Supply & Demand Zones")
    report_lines.append("**Status:** Completed (Dual-Year Validated)")
    report_lines.append("**Ruleset:** Bounce Reversion 3.0$\\sigma$ Target / 3.0$\\sigma$ Stop.")
    report_lines.append("")
    ...
    for year in ['2024', '2025']:
        ...
        for setup in [1, 2]:
            ...
            n = len(df_sub)
            desc = "Demand Zone Bounce" if setup == 1 else "Supply Zone Bounce"
    ...
    plot_path = os.path.join(assets_dir, 'DOC-ZONE-21_distributions.png')
    fig, axes = plt.subplots(1, 2, figsize=(12, 5))
    fig.suptitle('DOC-ZONE-21: Virgin Supply/Demand (Both Years)', fontsize=16)
    ...
    report_lines.append(f"![Distribution Plot](./DOC-ZONE-21_distributions.png)")
    
    report_path = os.path.join(os.path.dirname(__file__), 'DOC_21_Virgin_Supply_Demand.md')
    with open(report_path, 'w') as f:
        f.write("\n".join(report_lines))
        
    print(f"[ZONE-21 Deep Dive] Complete. Report written to {report_path}")
