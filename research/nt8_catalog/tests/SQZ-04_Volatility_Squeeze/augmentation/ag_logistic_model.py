...
def run_logistic_regression():
    ...
    if not os.path.exists(events_path):
        print(f"[SQZ-04_Volatility_Squeeze] No events.parquet found. Run level 1 deepdive first.")
        return
        
    df = pd.read_parquet(events_path)
    if len(df) < 10:
        print(f"[SQZ-04_Volatility_Squeeze] events.parquet has too few events (N={len(df)}).")
        return
        
    print(f"[SQZ-04_Volatility_Squeeze] Loaded {len(df)} events for Logistic Regression.")
    ...
    report_lines = []
    report_lines.append(f"# Logistic Regression DOE: SQZ-04_Volatility_Squeeze")
    ...
    print(f"[SQZ-04_Volatility_Squeeze] Complete. Report written to {report_path}")
