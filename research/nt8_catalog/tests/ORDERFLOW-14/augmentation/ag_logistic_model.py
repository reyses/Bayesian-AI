...
def run_logistic_regression():
    ...
    if not os.path.exists(events_path):
        print(f"[ORDERFLOW-14] No events.parquet found. Run level 1 deepdive first.")
        return
        
    df = pd.read_parquet(events_path)
    if len(df) < 10:
        print(f"[ORDERFLOW-14] events.parquet has too few events (N={len(df)}).")
        return
        
    print(f"[ORDERFLOW-14] Loaded {len(df)} events for Logistic Regression.")
    ...
    report_lines = []
    report_lines.append(f"# Logistic Regression DOE: ORDERFLOW-14")
    ...
    print(f"[ORDERFLOW-14] Complete. Report written to {report_path}")
