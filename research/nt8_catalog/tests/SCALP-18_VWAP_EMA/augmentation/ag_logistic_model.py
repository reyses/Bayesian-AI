...
def run_logistic_regression():
    ...
    if not os.path.exists(events_path):
        print(f"[SCALP-18_VWAP_EMA] No events.parquet found. Run level 1 deepdive first.")
        return
        
    df = pd.read_parquet(events_path)
    if len(df) < 10:
        print(f"[SCALP-18_VWAP_EMA] events.parquet has too few events (N={len(df)}).")
        return
        
    print(f"[SCALP-18_VWAP_EMA] Loaded {len(df)} events for Logistic Regression.")
    ...
    report_lines = []
    report_lines.append(f"# Logistic Regression DOE: SCALP-18_VWAP_EMA")
    ...
    print(f"[SCALP-18_VWAP_EMA] Complete. Report written to {report_path}")
