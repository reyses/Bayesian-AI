...
def run_logistic_regression():
    ...
    if not os.path.exists(events_path):
        print(f"[CROSS-11_Golden_Cross] No events.parquet found. Run level 1 deepdive first.")
        return
        
    df = pd.read_parquet(events_path)
    if len(df) < 10:
        print(f"[CROSS-11_Golden_Cross] events.parquet has too few events (N={len(df)}).")
        return
        
    print(f"[CROSS-11_Golden_Cross] Loaded {len(df)} events for Logistic Regression.")
    ...
    report_lines = []
    report_lines.append(f"# Logistic Regression DOE: CROSS-11_Golden_Cross")
    ...
    print(f"[CROSS-11_Golden_Cross] Complete. Report written to {report_path}")
