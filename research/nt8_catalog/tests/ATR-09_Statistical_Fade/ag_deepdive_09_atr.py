...
def process_day(args):
    ...
    daily_atr = yest_profile['high'] - yest_profile['low']
    if daily_atr <= 0: return None
    
    target_up = open_price + (1.0 * daily_atr)
    target_down = open_price - (1.0 * daily_atr)
...
if __name__ == '__main__':
    ...
    report_lines.append("**Title:** Deep Dive #9: Statistical ATR fade (90% rule)")
    report_lines.append("**Status:** Completed (Dual-Year Validated)")
    report_lines.append("**Ruleset:** Bespoke Exit (Revert 50% ATR or 10pt Stop). Unclamped Magnitude.")
    report_lines.append("")
    ...
    for year in ['2024', '2025']:
        ...
        for setup in [1, 2]:
            ...
            n = len(df_sub)
            desc = "Bearish Fade" if setup == 1 else "Bullish Fade"
    ...
    fig.suptitle('DOC-ATR-09: Realizable ATR Fade Setups (Both Years)', fontsize=16)
