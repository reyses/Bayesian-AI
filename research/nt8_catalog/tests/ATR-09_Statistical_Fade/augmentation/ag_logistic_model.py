...
def run_logistic_regression():
    ...
    if not os.path.exists(events_path):
        print(f"[ATR-09_Statistical_Fade] No events.parquet found. Run level 1 deepdive first.")
        return
        
    df = pd.read_parquet(events_path)
    if len(df) < 10:
        print(f"[ATR-09_Statistical_Fade] events.parquet has too few events (N={len(df)}).")
        return
        
    print(f"[ATR-09_Statistical_Fade] Loaded {len(df)} events for Logistic Regression.")
    ...
    report_lines = []
    report_lines.append(f"# Logistic Regression DOE: ATR-09_Statistical_Fade")
    ...
    print(f"[ATR-09_Statistical_Fade] Complete. Report written to {report_path}")
