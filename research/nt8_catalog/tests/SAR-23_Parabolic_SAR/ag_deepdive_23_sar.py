...
@numba.njit
def compute_psar(high, low, af_step=0.02, af_max=0.2):
    n = len(high)
    psar = np.zeros(n)
    is_bullish = np.ones(n, dtype=np.bool_)
    ...
    for i in range(1, n):
        ...
        if is_bullish[i-1]:
            ...
            if i >= 2:
                ...
                curr_psar = min(curr_psar, low[i-1])
            ...
            # Bearish
            curr_psar = prev_psar - af * (prev_psar - ep)
            if i >= 2:
                curr_psar = max(curr_psar, high[i-1], high[i-2])
            else:
                curr_psar = max(curr_psar, high[i-1])
...
def process_day(day):
    ...
    try:
        df = pd.read_parquet(parquet_path, columns=['open', 'high', 'low', 'close', 'timestamp'])
    except Exception:
        return None
    ...
    # Calculate Parabolic SAR across the whole day 
    psar, is_bullish = compute_psar(df['high'].values, df['low'].values)
    df['psar'] = psar
    ...
    bullish = df_day['is_bullish'].values
    sigmas = df_day['sigma'].values
    
    events_found = []
    cooldown = 0
    
    for i in range(1, len(cl) - 60):
        if cooldown > 0:
            cooldown -= 1
            continue
        ...
        # Setup 1: SAR Flip to Bullish
        if not bullish[i-1] and bullish[i]:
            setup = 1
            mode = 'bullish_flip'
            event_idx = i
            
        # Setup 2: SAR Flip to Bearish
...
if __name__ == '__main__':
    ...
    print(f"[SAR-23 Deep Dive] Evaluating Setups across {len(days)} days...")
    tasks = days
    ...
    if len(all_events) == 0:
        print("[SAR-23 Deep Dive] No events found.")
    ...
    print(f"[SAR-23 Deep Dive] Extracted {len(df)} triggered events.")
    ...
    report_lines = []
    report_lines.append("# Document ID: AG-DOC-SAR-23")
    report_lines.append("**Title:** Deep Dive #23: Parabolic SAR Flip")
    report_lines.append("**Status:** Completed (Dual-Year Validated)")
    report_lines.append("**Ruleset:** Trend Flip 3.0$\\sigma$ Target / 3.0$\\sigma$ Stop.")
    report_lines.append("")
    ...
    for year in ['2024', '2025']:
        ...
        for setup in [1, 2]:
            ...
            n = len(df_sub)
            desc = "Bullish Flip" if setup == 1 else "Bearish Flip"
    ...
    plot_path = os.path.join(assets_dir, 'DOC-SAR-23_distributions.png')
    fig, axes = plt.subplots(1, 2, figsize=(12, 5))
    fig.suptitle('DOC-SAR-23: Parabolic SAR (Both Years)', fontsize=16)
    ...
    report_lines.append(f"![Distribution Plot](./DOC-SAR-23_distributions.png)")
    
    report_path = os.path.join(os.path.dirname(__file__), 'DOC_23_Parabolic_SAR.md')
    with open(report_path, 'w') as f:
        f.write("\n".join(report_lines))
        
    print(f"[SAR-23 Deep Dive] Complete. Report written to {report_path}")
