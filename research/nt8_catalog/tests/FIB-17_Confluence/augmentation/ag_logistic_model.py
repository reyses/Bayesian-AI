...
def run_logistic_regression():
    ...
    if not os.path.exists(events_path):
        print(f"[FIB-17_Confluence] No events.parquet found. Run level 1 deepdive first.")
        return
        
    df = pd.read_parquet(events_path)
    if len(df) < 10:
        print(f"[FIB-17_Confluence] events.parquet has too few events (N={len(df)}).")
        return
        
    print(f"[FIB-17_Confluence] Loaded {len(df)} events for Logistic Regression.")
    ...
    report_lines = []
    report_lines.append(f"# Logistic Regression DOE: FIB-17_Confluence")
    ...
    print(f"[FIB-17_Confluence] Complete. Report written to {report_path}")
