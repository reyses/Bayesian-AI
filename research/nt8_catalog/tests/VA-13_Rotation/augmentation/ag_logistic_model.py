...
def run_logistic_regression():
    ...
    if not os.path.exists(events_path):
        print(f"[VA-13_Rotation] No events.parquet found. Run level 1 deepdive first.")
        return
        
    df = pd.read_parquet(events_path)
    if len(df) < 10:
        print(f"[VA-13_Rotation] events.parquet has too few events (N={len(df)}).")
        return
        
    print(f"[VA-13_Rotation] Loaded {len(df)} events for Logistic Regression.")
    ...
    report_lines = []
    report_lines.append(f"# Logistic Regression DOE: VA-13_Rotation")
    ...
    print(f"[VA-13_Rotation] Complete. Report written to {report_path}")
