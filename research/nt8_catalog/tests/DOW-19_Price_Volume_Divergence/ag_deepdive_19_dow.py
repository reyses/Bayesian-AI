...
def process_day(day):
    ...
    # 20-period SMA of price (for mean reversion target)
    df['price_sma20'] = df['close'].rolling(20).mean().bfill()
    
    # Local extremes (10 bar breakout)
    ...
    vol_sma20 = df_day['vol_sma20'].values
    price_sma20 = df_day['price_sma20'].values
    high_10 = df_day['high_10'].values
    low_10 = df_day['low_10'].values
    ...
    for i in range(20, len(prices) - 60):
        if cooldown > 0:
            cooldown -= 1
            continue
            
        # Setup 1: Bearish Divergence Trap (Price breaks out high, but volume is falling below average)
        if prices[i] > high_10[i] and volumes[i] < vol_sma20[i]:
            setup = 1
            mode = 'bearish_trap'
            event_idx = i
            
        # Setup 2: Bullish Divergence Trap (Price breaks down low, but volume is falling below average)
        elif prices[i] < low_10[i] and volumes[i] < vol_sma20[i]:
            setup = 2
            mode = 'bullish_trap'
            event_idx = i
        ...
        if event_idx != -1:
            p0 = prices[event_idx]
            path = prices[event_idx+1 : event_idx+61] # 5 mins max hold
            target_path = price_sma20[event_idx+1 : event_idx+61]
            ...
            if mode == 'bullish_trap':
                for p, t, std in zip(path, target_path, std_path):
                    if p >= t: # Target hit (mean revert up to SMA)
                        magnitude = p - p0
                        hit_target = True
                        break
            ...
            elif mode == 'bearish_trap':
                for p, t, std in zip(path, target_path, std_path):
                    if p <= t: # Target hit (mean revert down to SMA)
                        magnitude = p0 - p
                        hit_target = True
                        break
                    elif p >= p0 + 3.0 * std: # Stop hit
        ...
        'duration_bars': _exit_idx if '_exit_idx' in locals() else -1,
            ...
            cooldown = 60 # wait 5 mins before next event
            event_idx = -1
...
if __name__ == '__main__':
    ...
    print(f"[DOW-19 Deep Dive] Evaluating Setups across {len(days)} days...")
    tasks = days
    ...
    if len(all_events) == 0:
        print("[DOW-19 Deep Dive] No events found.")
    ...
    print(f"[DOW-19 Deep Dive] Extracted {len(df)} triggered events.")
    ...
    report_lines = []
    report_lines.append("# Document ID: AG-DOC-DOW-19")
    report_lines.append("**Title:** Deep Dive #19: Dow Price-Volume Divergence")
    report_lines.append("**Status:** Completed (Dual-Year Validated)")
    report_lines.append("**Ruleset:** Exit (Mean Revert to SMA-20 or 3.0$\\sigma$ Stop).")
    report_lines.append("")
    ...
    for year in ['2024', '2025']:
        ...
        for setup in [1, 2]:
            ...
            n = len(df_sub)
            desc = "Bearish Trap" if setup == 1 else "Bullish Trap"
    ...
    plot_path = os.path.join(assets_dir, 'DOC-DOW-19_distributions.png')
    fig, axes = plt.subplots(1, 2, figsize=(12, 5))
    fig.suptitle('DOC-DOW-19: Dow Volume Divergence (Both Years)', fontsize=16)
    ...
    report_lines.append(f"![Distribution Plot](./DOC-DOW-19_distributions.png)")
    
    report_path = os.path.join(os.path.dirname(__file__), 'DOC_19_Price_Volume_Divergence.md')
    with open(report_path, 'w') as f:
        f.write("\n".join(report_lines))
        
    print(f"[DOW-19 Deep Dive] Complete. Report written to {report_path}")
