...
def run_logistic_regression():
    ...
    if not os.path.exists(events_path):
        print(f"[ADX-08_Trend_Gate] No events.parquet found. Run level 1 deepdive first.")
        return
        
    df = pd.read_parquet(events_path)
    if len(df) < 10:
        print(f"[ADX-08_Trend_Gate] events.parquet has too few events (N={len(df)}).")
        return
        
    print(f"[ADX-08_Trend_Gate] Loaded {len(df)} events for Logistic Regression.")
    ...
    report_lines = []
    report_lines.append(f"# Logistic Regression DOE: ADX-08_Trend_Gate")
    ...
    print(f"[ADX-08_Trend_Gate] Complete. Report written to {report_path}")
