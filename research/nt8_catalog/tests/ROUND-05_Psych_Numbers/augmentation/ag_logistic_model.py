...
def run_logistic_regression():
    ...
    if not os.path.exists(events_path):
        print(f"[ROUND-05_Psych_Numbers] No events.parquet found. Run level 1 deepdive first.")
        return
        
    df = pd.read_parquet(events_path)
    if len(df) < 10:
        print(f"[ROUND-05_Psych_Numbers] events.parquet has too few events (N={len(df)}).")
        return
        
    print(f"[ROUND-05_Psych_Numbers] Loaded {len(df)} events for Logistic Regression.")
    ...
    report_lines = []
    report_lines.append(f"# Logistic Regression DOE: ROUND-05_Psych_Numbers")
    ...
    print(f"[ROUND-05_Psych_Numbers] Complete. Report written to {report_path}")
