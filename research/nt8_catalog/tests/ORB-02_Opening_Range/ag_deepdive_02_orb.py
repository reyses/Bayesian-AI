...
def process_day(day):
    ...
    # Filter for RTH (08:30 to 15:15 CT) as bounding
    df['dt'] = pd.to_datetime(df['timestamp'], unit='s', utc=True).dt.tz_convert('America/Chicago')
    
    # 1. OR between exactly 08:30:00 CT and 08:59:59 CT
    df_or = df[(df['dt'].dt.time >= pd.Timestamp('08:30:00').time()) & (df['dt'].dt.time < pd.Timestamp('09:00:00').time())]
    if len(df_or) == 0: return None
    
    # 2. Calculate OR_High and OR_Low
    or_high = df_or['close'].max()
    or_low = df_or['close'].min()
    
    # 3. Trading session starts at 09:00:00 CT
    df_session = df[(df['dt'].dt.time >= pd.Timestamp('09:00:00').time()) & (df['dt'].dt.time <= pd.Timestamp('15:15:00').time())].copy()
    if len(df_session) == 0: return None
    
    prices = df_session['close'].values
    times = df_session['dt'].values
...
if __name__ == '__main__':
    ...
    print(f"[ORB Deep Dive] Evaluating Setups for {len(days)} days...")
    ...
    print(f"[ORB Deep Dive] Extracted {len(df)} triggered events.")
    ...
    report_lines = []
    report_lines.append("# Document ID: AG-DOC-ORB-02 (LOGISTIC REGRESSION VERIFIED)")
    report_lines.append("**Title:** Deep Dive #2: 30-min Opening Range Break (ORB)")
    report_lines.append("**Status:** Completed (Dual-Year Validated)")
    report_lines.append("**Ruleset:** Bespoke Exit (OR Midpoint or EOD). Unclamped Magnitude.")
    report_lines.append("")
    ...
    for year in ['2024', '2025']:
        ...
        for setup in [1, 2]:
            ...
            n = len(df_sub)
            desc = "Bullish Breakout" if setup == 1 else "Bearish Breakout"
    ...
    plot_path = os.path.join(assets_dir, 'DOC-ORB-02_distributions.png')
    fig, axes = plt.subplots(1, 2, figsize=(12, 5))
    fig.suptitle('DOC-ORB-02: Realizable ORB Setups (Both Years)', fontsize=16)
    ...
    report_lines.append(f"![Distribution Plot](./DOC-ORB-02_distributions.png)")
    
    report_path = os.path.join(os.path.dirname(__file__), 'DOC_ORB_02.md')
    with open(report_path, 'w') as f:
        f.write("\n".join(report_lines))
        
    print(f"[ORB Deep Dive] Complete. Report written to {report_path}")
