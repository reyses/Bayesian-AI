...
def run_logistic_regression():
    ...
    if not os.path.exists(events_path):
        print(f"[ORB-02_Opening_Range] No events.parquet found. Run level 1 deepdive first.")
        return
        
    df = pd.read_parquet(events_path)
    if len(df) < 10:
        print(f"[ORB-02_Opening_Range] events.parquet has too few events (N={len(df)}).")
        return
        
    print(f"[ORB-02_Opening_Range] Loaded {len(df)} events for Logistic Regression.")
    ...
    report_lines = []
    report_lines.append(f"# Logistic Regression DOE: ORB-02_Opening_Range")
    ...
    print(f"[ORB-02_Opening_Range] Complete. Report written to {report_path}")
