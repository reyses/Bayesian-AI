...
def run_logistic_regression():
    ...
    if not os.path.exists(events_path):
        print(f"[PIVOT-16_Floor_Levels] No events.parquet found. Run level 1 deepdive first.")
        return
        
    df = pd.read_parquet(events_path)
    if len(df) < 10:
        print(f"[PIVOT-16_Floor_Levels] events.parquet has too few events (N={len(df)}).")
        return
        
    print(f"[PIVOT-16_Floor_Levels] Loaded {len(df)} events for Logistic Regression.")
    ...
    report_lines = []
    report_lines.append(f"# Logistic Regression DOE: PIVOT-16_Floor_Levels")
    ...
    print(f"[PIVOT-16_Floor_Levels] Complete. Report written to {report_path}")
