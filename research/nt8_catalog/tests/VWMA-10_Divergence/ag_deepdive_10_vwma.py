...
def process_day(day):
    ...
    # Compute VWMA and SMA (240 periods on 5s data = 20 minutes)
    df['pv'] = df['close'] * df['volume']
    df['vwma'] = df['pv'].rolling(240).sum() / df['volume'].rolling(240).sum()
    df['sma'] = df['close'].rolling(240).mean()
    ...
    vwmas = df_day['vwma'].values
    smas = df_day['sma'].values
    open_price = prices[0]
    ...
    # Setup 1 (Bullish Divergence / Cross): Scan forward from 08:30. If VWMA crosses ABOVE SMA
    # Setup 2 (Bearish Divergence / Cross): If VWMA crosses BELOW SMA
    for i in range(1, len(df_day)):
        if pd.isna(vwmas[i]) or pd.isna(smas[i]) or pd.isna(vwmas[i-1]) or pd.isna(smas[i-1]):
            continue
            
        if vwmas[i] > smas[i] and vwmas[i-1] <= smas[i-1]:
            setup = 1
            event_idx = i
            mode = 'bullish_runner'
            break
        elif vwmas[i] < smas[i] and vwmas[i-1] >= smas[i-1]:
            setup = 2
            event_idx = i
            mode = 'bearish_runner'
            break
    ...
    vwmas_path = vwmas[event_idx+1 :]
    smas_path = smas[event_idx+1 :]
    ...
    if mode in ['bullish_bounce', 'bullish_runner']:
        for p, v, s in zip(path, vwmas_path, smas_path):
            if v <= s: # VWMA crosses back below SMA
                magnitude = p - p0
        ...
        for p, v, s in zip(path, vwmas_path, smas_path):
            if v >= s: # VWMA crosses back above SMA
                magnitude = p0 - p
                hit_target = magnitude > 0
                break
        if magnitude == 0.0:
...
if __name__ == '__main__':
    ...
    print(f"[VWMA Deep Dive] Evaluating Setups for {len(days)} days...")
    ...
    print(f"[VWMA Deep Dive] Extracted {len(df)} triggered events.")
    ...
    report_lines = []
    report_lines.append("# Document ID: AG-DOC-10 (LOGISTIC REGRESSION VERIFIED)")
    report_lines.append("**Title:** Deep Dive #10: VWMA vs SMA divergence")
    report_lines.append("**Status:** Completed (Dual-Year Validated)")
    report_lines.append("**Ruleset:** Bespoke Exit (VWMA Reverse Crosses SMA). Unclamped Magnitude.")
    report_lines.append("")
    ...
    for year in ['2024', '2025']:
        ...
        for setup in [1, 2]:
            ...
            n = len(df_sub)
            desc = "Bullish Crossover" if setup == 1 else "Bearish Crossover"
    ...
    plot_path = os.path.join(assets_dir, 'DOC-10_distributions.png')
    fig, axes = plt.subplots(1, 2, figsize=(12, 5))
    fig.suptitle('DOC-10: VWMA vs SMA Setups (Both Years)', fontsize=16)
    ...
    for i, setup in enumerate([1, 2]):
        ...
        if len(df_sub) > 0:
            ...
            desc = "Bullish Runner" if setup == 1 else "Bearish Runner"
    ...
    report_lines.append(f"![Distribution Plot](./DOC-10_distributions.png)")
    
    report_path = os.path.join(os.path.dirname(__file__), 'DOC_10.md')
    with open(report_path, 'w') as f:
        f.write("\n".join(report_lines))
        
    print(f"[VWMA Deep Dive] Complete. Report written to {report_path}")
