...
def run_logistic_regression():
    ...
    if not os.path.exists(events_path):
        print(f"[VWMA-10_Divergence] No events.parquet found. Run level 1 deepdive first.")
        return
        
    df = pd.read_parquet(events_path)
    if len(df) < 10:
        print(f"[VWMA-10_Divergence] events.parquet has too few events (N={len(df)}).")
        return
        
    print(f"[VWMA-10_Divergence] Loaded {len(df)} events for Logistic Regression.")
    ...
    report_lines = []
    report_lines.append(f"# Logistic Regression DOE: VWMA-10_Divergence")
    ...
    print(f"[VWMA-10_Divergence] Complete. Report written to {report_path}")
