...
class BayesianAtlasDataset(Dataset):
    ...
    """
    def __init__(self, features_dir: str, date_str: str, seq_len: int = 100, atr_mult: float = 4.0, min_bars_5s: int = 36, tick_size: float = 0.25, expected_columns: list = None):
    ...
    def _load_and_join_day(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # Enforce canonical columns if provided to avoid dimensionality crashes (e.g., missing parquet files on holidays)
        if self.expected_columns is not None:
            # Reindex will add missing columns with NaN and drop extra columns to match exact order
            joined_df = joined_df.reindex(columns=self.expected_columns, fill_value=0.0)
        else:
            # First time loading: set the expected columns implicitly so the caller can extract them
            self.expected_columns = joined_df.columns.tolist()
