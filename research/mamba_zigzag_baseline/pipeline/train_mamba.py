...
def train_autonomous_engine(args):
    ...
    expected_columns = peek_dataset.expected_columns
    del peek_dataset
    gc.collect()
    ...
    for current_idx, date_str in enumerate(batch_dates, start=start_idx):
        ...
        try:
            dataset = BayesianAtlasDataset(features_dir=base_dir, date_str=date_str, seq_len=100, expected_columns=expected_columns)
