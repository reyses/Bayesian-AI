...
# Corrected break rule (MUST match stage1_speed_pass.py): per-bar tolerance, consecutive-off break.
BAND_FRAC = 0.10
TICK_FLOOR = 0.25
BREAK_CONSEC = 5
MAX_TIER = 10   # tier t -> band = (10%*t) of the per-bar move; tier = loosening needed to model; 99 = PURE_CHAOS

def _seg_tier(residuals, delta_abs):
    """Graded tier (same 5-consecutive rule as pristine, looser band each step):
    smallest t in 1..MAX_TIER s.t. the fit holds (<BREAK_CONSEC consecutive off) at band = 10%*t * |per-bar move|
    (floored at 1 tick). Tier 1 = fits at 10% (pristine-grade); higher = needs a looser band; 99 = nothing to model."""
    res = np.atleast_1d(residuals); d = np.abs(delta_abs)
    for t in range(1, MAX_TIER + 1):
        thr = np.maximum(0.10 * t * d, TICK_FLOOR)
        if max_consecutive(res > thr) < BREAK_CONSEC:
            return t
    return 99
...
def batched_ols_scan_pytorch(X_t, Y_t, min_bars, max_bars, E, delta_abs_win=None):
    device = X_t.device
    p = X_t.shape[1]
    ...
    for i, L in enumerate(range(min_bars, max_bars + 1)):
        ...
        tier = _seg_tier(res_cpu, delta_abs_win[:L])
...
def evaluate_block(start_idx, length, E, X_global_t, close_prices_t, groups):
    ...
    delta_seg = np.abs(np.diff(raw_np, prepend=raw_np[0]))
    tier = _seg_tier(residuals, delta_seg)
    ...
    while seed_start + SEED_BARS < N_GAP:
        ...
        if tier <= MAX_TIER:
            ...
            if max_forward > SEED_BARS:
                ...
                raw_exp = close_prices_t[seed_start:seed_start+max_forward].cpu().numpy()
                delta_win = np.abs(np.diff(raw_exp, prepend=raw_exp[0]))
                tiers_exp, max_res_exp, betas_exp = batched_ols_scan_pytorch(X_exp_poly_t, Y_exp_t, SEED_BARS + 1, max_forward, E, delta_win)
...
def main():
    ...
    print(f"[MAIN] Stage 2 Chaos Processor starting for {args.day} | Run: {args.run_name} | TF: {args.tf}")
    
    with open(f"artifacts/stage1_{args.run_name}_segments_{args.day}.json", 'r') as f:
    ...
    try:
        if "RUN_A" in features_root or "RUN_B" in features_root or "RUN_C" in features_root:
            df = pd.read_parquet(os.path.join(features_root, f'{args.day}.parquet'))
        else:
            df = load_features([args.day], root=features_root, require_all=False)
    except Exception as e:
        print(f"[FATAL] Feature load failed: {e}")
        return
        
    ohlcv = pd.read_parquet(os.path.join(atlas_root, args.tf, f'{args.day}.parquet'))
    ...
    output_json = f"artifacts/stage2_{args.run_name}_segments_{args.day}.json"
