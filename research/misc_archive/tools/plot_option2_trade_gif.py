...
def main():
    # Load trades
    trades_csv = 'C:/Users/reyse/OneDrive/Desktop/Bayesian-AI/reports/findings/nmp_triple_timeline_option2.csv'
    tr = pd.read_csv(trades_csv)
    
    # Let's pick the best trade to see a massive run
    ...
    # Kinematics for Pink Line (15m span = 900 points)
    span_15 = 900 
    ...
    for i in range(blue_sample_size, len(df_calc), 15): # step by 15 for speed
    ...
    for i in range(orange_sample_size, len(df_calc), 15):
        curr_ts = times_calc[i-1]
        if curr_ts < start_ts:
            continue
    ...
    frames = np.arange(0, len(df_zoom), 15)
    ...
    def update(frame):
        ...
        if current_ts >= ets:
            ax.axvline(x=entry_rel, color='green', linestyle='--', label='ENTRY (LONG)')
            entry_idx = np.where(timestamps_1s == ets)[0][0]
            if entry_idx <= idx_1s:
                ax.scatter([entry_rel], [prices_1s[entry_idx]], color='green', s=100, zorder=10)
        if current_ts >= xts:
            ax.axvline(x=exit_rel, color='red', linestyle='--', label='EXIT')
            exit_idx = np.where(timestamps_1s == xts)[0][0]
            if exit_idx <= idx_1s:
                ax.scatter([exit_rel], [prices_1s[exit_idx]], color='red', s=100, zorder=10)
        ...
        ax.set_title(f"Trade Execution | PnL: ${trade['net_usd']:.2f} | Pink_V: {v_15*60:+.2f}", fontweight='bold')
        ax.set_ylabel("Price")
    ...
    gif_path = "C:/Users/reyse/.gemini/antigravity/brain/0b405af3-d525-4c87-b71d-cb77ea225a55/triple_timeline_trade_execution.gif"
