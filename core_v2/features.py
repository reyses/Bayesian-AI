...
_ASSEMBLE_V2_IDX: Optional[np.ndarray] = None


def _build_assemble_v2_idx() -> np.ndarray:
    """Build the (N_TFS_V2 * N_FEATURES_PER_TF_V2,) flat-index map once.

    Same name-keyed placement loop as before (the Anti-Scramble guarantee);
    only the *lookup* is hoisted out of the per-call path. FEATURE_NAMES is
    module-level and immutable at runtime, so the map never goes stale.
    """
    assert TF_HIERARCHY_V2[0] == '1D', "TF_HIERARCHY_V2 must be descending!"
    name_to_idx = {name: j for j, name in enumerate(FEATURE_NAMES)}
    idx = np.empty(N_TFS_V2 * N_FEATURES_PER_TF_V2, dtype=np.int64)
    k = 0
...
def assemble_v2_grid(flat_v2_matrix: np.ndarray) -> np.ndarray:
    ...
    The name-keyed mapping is built once (see _build_assemble_v2_idx) and applied
    as a single vectorized gather — bit-identical output to the per-call loop.
    ...
    """
    global _ASSEMBLE_V2_IDX
    if _ASSEMBLE_V2_IDX is None:
        _ASSEMBLE_V2_IDX = _build_assemble_v2_idx()

    N = flat_v2_matrix.shape[0]
    grid = flat_v2_matrix[:, _ASSEMBLE_V2_IDX].astype(np.float32, copy=False)
    return grid.reshape(N, N_TFS_V2, N_FEATURES_PER_TF_V2)
